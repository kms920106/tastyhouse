...
- **backend 모듈** (`backend/settings.gradle`): 총 24개이며 세 갈래입니다.
  ...
  - **공유 모듈**: `domain`, `infrastructure:persistence`, `infrastructure:redis`, `infrastructure:{external,file-storage,firebase,aws-s3,aws-ses,aws-sns,oauth,payment,messaging,bbq,admdongkor}`, `security-module`, `logging-module`, `api-common-module`.

  읽기 계약(`com.tastyhouse.application..port.out`의 `{Ctx}QueryPort`·`*Result`·`*SearchCondition`)을 담던 `application-common-module`은 계약이 전부 소유 모듈로 옮겨가며 **삭제됐습니다**(18 → 17개). 이후 앱별 application 모듈 4개를 `application` 하나로 합쳐 14개가 됐고, 다시 `infrastructure:external`을 벤더·채널 7모듈로 나눠 20개가 됐고, 파일 저장 스타터 `infrastructure:file-storage`가 신설돼 21개가 됐습니다. 이후 `infrastructure:aws`가 `aws-s3`·`aws-ses`·`aws-sns` 3모듈로 분할돼 23개가 됐고, 배치 전용 수집 모듈 `infrastructure:crawling`이 `bbq`(BBQ 메뉴)·`admdongkor`(행정동 경계) 2모듈로 분할돼 **24개**가 됐습니다. 읽기 계약은 전부 `application` 모듈이 소유합니다 — 공유 계약 55개를 잠시 `domain`이 갖고 있었으나 application 모듈 통합으로 그 근거(앱 간 수평 의존 회피)가 사라져 되돌렸고, 그 결과 `com.tastyhouse.application` 최상위 패키지를 한 모듈이 단독으로 갖습니다.

  `infrastructure`는 기술별로 나뉜 **중첩 프로젝트**입니다(디렉터리 `backend/infrastructure/{persistence,redis,external,file-storage,firebase,aws-s3,aws-ses,aws-sns,oauth,payment,messaging,bbq,admdongkor}/`, Gradle 좌표는 각각 `:infrastructure:{이름}`). 13개 중 12개가 driven(아웃바운드) 어댑터이고, `file-storage`만 자바 코드 없이 파일 저장 벤더 구현(현재 firebase)을 묶어 앱에 노출하는 조립 전용 스타터입니다. 자바 패키지는 persistence·redis가 `com.tastyhouse.infrastructure..`로 불변이고, **외부 연동 11모듈 중 자바 코드가 있는 10개만 예외로 `com.tastyhouse.external..`을 그대로 유지합니다**(모듈마다 하위 패키지가 겹치지 않게 나눠 소유. `file-storage`는 자바 코드가 없어 소유할 패키지 자체가 없습니다) — `InfrastructureModuleConfig`가 `com.tastyhouse.infrastructure`를 통째로 스캔해서, 그 아래로 옮기면 빈 스캔 범위가 어긋나 admin/ceo/batch 부팅이 깨지기 때문입니다.

  외부 연동 11모듈은 **앱별 실사용 차이**로 나뉩니다 — `external`(HTTP 코어: WebClient·외부 연동 예외. 파일 저장과는 무관 — 벤더 구현이 도메인 포트 `FileStoragePort`를 직접 구현합니다) / `file-storage`(파일 저장 스타터 — 코드 없이 벤더 구현 firebase를 조립, **4앱이 직접 의존하는 유일한 파일 저장 좌표**) / `firebase`(앱이 아니라 `file-storage`가 의존) / `oauth`·`payment`·`messaging`(web만) / `bbq`·`admdongkor`(batch만) / `aws-s3`·`aws-ses`·`aws-sns`(**어느 앱도 의존하지 않고 컴파일만 검증** — provider 기본값이 전부 비-AWS. 전환 절차는 각각 `backend/infrastructure/aws-s3/AGENTS.md`·`backend/infrastructure/aws-ses/AGENTS.md`·`backend/infrastructure/aws-sns/AGENTS.md`). 모듈 지도와 배치 기준은 `backend/CLAUDE.md`의 "모듈 지도" 절을 참조합니다.
