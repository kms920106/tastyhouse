...
브라우저 동작 확인이 필요한 frontend 작업(화면/폼/플로우 구현 및 수정)을 완료했을 때는, 코드 구현 → 빌드/린트 → 코드리뷰만으로 끝내지 말고 **직접 개발 서버를 기동해 MCP Playwright로 실제 화면 동작까지 검증**합니다. 빌드·린트 통과(admin/ceo는 `npm run build`·`npm run check`, web은 `npm run build`·`npm run code-quality`)는 "테스트 스위트가 없으니 빌드가 최소 게이트"라는 의미일 뿐, 브라우저 검증을 생략해도 된다는 뜻이 아닙니다. 절차:

1. 개발 서버가 떠 있는지 확인하고, 없으면 **해당 앱 디렉터리로 이동해** `npm run dev`를 백그라운드로 기동합니다. 앱별 디렉터리와 포트는 아래 [개발 서버 포트](#개발-서버-포트)를 참조합니다.
2. 로그인이 필요하면 **해당 앱 디렉터리의** `.env.local`에 있는 `E2E_USERNAME`/`E2E_PASSWORD`로 로그인합니다.
...
5. 검증이 끝나면 **이번 세션에서 검증을 위해 직접 기동한** 개발 서버·백엔드 프로세스(위 [개발 서버 포트](#개발-서버-포트)의 해당 포트)를 종료합니다. 검증 시작 전에 이미 떠 있던(사용자가 별도로 기동해 둔) 서버는 종료하지 않습니다.
...
브라우저 동작 확인이 필요한 frontend 작업(화면/폼/플로우 구현 및 수정)의 플랜을 작성할 때는, **구현 전에 MCP Playwright로 검증할 e2e 시나리오를 플랜에 반드시 명시**합니다. 이 시나리오는 사후 검증(위 "AI 규칙"의 MCP Playwright 검증 절차) 단계에서 그대로 수행 대상이 됩니다. 순수 백엔드/설정/리팩터링/문서 등 브라우저 조작이 필요 없는 작업은 이 항목이 면제됩니다. 시나리오는 다음을 포함합니다.
