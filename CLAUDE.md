...
- **backend 모듈** (`backend/settings.gradle`): 총 20개이며 세 갈래입니다.
  ...
  - **공유 모듈**: `domain-module`, `infrastructure:persistence`, `infrastructure:redis`, `infrastructure:{external,firebase,aws,oauth,payment,messaging,crawling}`, `security-module`, `logging-module`, `api-common-module`.

  읽기 계약(`com.tastyhouse.application..port.out`의 `{Ctx}QueryPort`·`*Result`·`*SearchCondition`)을 담던 `application-common-module`은 계약이 전부 소유 모듈로 옮겨가며 **삭제됐습니다**(18 → 17개). 이후 앱별 application 모듈 4개를 `application` 하나로 합쳐 14개가 됐고, 다시 `infrastructure:external`을 벤더·채널 7모듈로 나눠 **20개**가 됐습니다. 읽기 계약은 전부 `application` 모듈이 소유합니다 — 공유 계약 55개를 잠시 `domain-module`이 갖고 있었으나 application 모듈 통합으로 그 근거(앱 간 수평 의존 회피)가 사라져 되돌렸고, 그 결과 `com.tastyhouse.application` 최상위 패키지를 한 모듈이 단독으로 갖습니다.

  `infrastructure`는 기술별로 나뉜 **중첩 프로젝트**입니다(디렉터리 `backend/infrastructure/{persistence,redis,external,firebase,aws,oauth,payment,messaging,crawling}/`, Gradle 좌표는 각각 `:infrastructure:{이름}`). 9개 전부 driven(아웃바운드) 어댑터입니다. 자바 패키지는 persistence·redis가 `com.tastyhouse.infrastructure..`로 불변이고, **외부 연동 7모듈만 예외로 `com.tastyhouse.external..`을 그대로 유지합니다**(모듈마다 하위 패키지가 겹치지 않게 나눠 소유) — `InfrastructureModuleConfig`가 `com.tastyhouse.infrastructure`를 통째로 스캔해서, 그 아래로 옮기면 빈 스캔 범위가 어긋나 admin/ceo/batch 부팅이 깨지기 때문입니다.

  외부 연동 7모듈은 **앱별 실사용 차이**로 나뉩니다 — `external`(코어: WebClient·예외·파일 저장 SPI) / `firebase`(4앱 전부) / `oauth`·`payment`·`messaging`(web만) / `crawling`(batch만) / `aws`(**어느 앱도 의존하지 않고 컴파일만 검증** — provider 기본값이 전부 비-AWS. 전환 절차는 `backend/infrastructure/aws/AGENTS.md`). 모듈 지도와 배치 기준은 `backend/CLAUDE.md`의 "모듈 지도" 절을 참조합니다.
