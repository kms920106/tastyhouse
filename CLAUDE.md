...
**backend의 `*.java`에는 주석을 쓰지 않습니다.** Javadoc(`/** */`)·라인 주석(`// `)·블록 주석(`/* */`) 전부가 대상이고 main·test를 가리지 않습니다.

설명할 것이 있으면 **처음부터 문서에 씁니다.** 설계 근거·금지 지시·도메인 규칙이 사는 곳은 코드가 아니라 **그 코드를 소유한 모듈의 `AGENTS.md`**(`## 봉인·가드 목록` / `## 코드 주석에서 이관된 설계 근거` 절)와 **`docs/domain/{도메인}.md`** 입니다. 주석으로 먼저 적었다가 나중에 옮기는 것이 아니라, **애초에 그 문서에 적는 것**이 이 규칙입니다. 같은 설명이 주석과 문서 양쪽에 있으면 한쪽만 갱신돼 두 벌이 갈라지기 때문입니다.

- **주석을 되살리지 않습니다.** 새 파일을 쓸 때도, 기존 파일을 고칠 때도 마찬가지입니다. 주석 없는 java 파일은 문서화 누락이 아니라 **의도된 상태**입니다.
- 문서에 적을 때는 **역참조 앵커 3요소**(리포 루트 기준 파일 경로 · 코드 요소명 · 원문 취지)를 채워 어느 코드를 가리키는지 밝힙니다. **줄 번호는 쓰지 않습니다** — 코드가 바뀌면 즉시 틀립니다.
- **주석이 아닌 것을 주석으로 오인해 지우지 않습니다** — ArchUnit `.because("...")` 44건, `@DisplayName` 186개 파일, `// noinspection` 1건, 각종 어노테이션·로그 메시지는 컴파일·런타임·정적분석에 관여합니다. 그래서 **backend 전체 잔존 주석 검사의 기대값은 0이 아니라 `1`** 입니다.

**예외적으로 남아 있는 주석을 발견했다면**(이관에서 누락됐거나 규칙을 모르는 채 추가된 것) 그때는 **먼저 그 내용을 문서에 옮겨 적고, 그다음 주석을 지웁니다.** 순서를 뒤집으면 무엇을 옮겨야 했는지가 사라집니다. 이것은 신규 작성 규칙이 아니라 **잔존 주석을 정리할 때만 적용되는 절차**입니다.

분류 기준(A 가드 / B 아키텍처 / C 도메인 규칙 / D 미이관 삭제), 검사 명령과 zsh 인용 함정, 감수한 손실(`{@link}` 1,126건 소실)은 `backend/CLAUDE.md`의 "java 주석 금지 규칙" 절에 있습니다. `application*.yml`·`build.gradle`은 단위 라벨·블록 제목 같은 예외를 두므로 같은 파일의 "설정·빌드 스크립트 주석 규칙" 절을 따릅니다. **frontend는 이 규칙의 대상이 아닙니다.**
...
- **backend 모듈** (`backend/settings.gradle`): 총 28개이며 세 갈래입니다.
  ...
  - **공유 모듈**: `domain`, `infrastructure:persistence`, `infrastructure:redis`, `infrastructure:{restclient,file-storage,firebase,aws-s3,aws-ses,aws-sns,oauth,pg,tosspayments,mail,javamail,sms,solapi,bbq,admdongkor}`, `security-module`, `logging-module`, `api-common-module`.

  읽기 계약(`com.tastyhouse.application..port.out`의 `{Ctx}QueryPort`·`*Result`·`*SearchCondition`)을 담던 `application-common-module`은 계약이 전부 소유 모듈로 옮겨가며 **삭제됐습니다**(18 → 17개). 이후 앱별 application 모듈 4개를 `application` 하나로 합쳐 14개가 됐고, 다시 `infrastructure:external`(현 `infrastructure:restclient`)을 벤더·채널 7모듈로 나눠 20개가 됐고, 파일 저장 스타터 `infrastructure:file-storage`가 신설돼 21개가 됐습니다. 이후 `infrastructure:aws`가 `aws-s3`·`aws-ses`·`aws-sns` 3모듈로 분할돼 23개가 됐고, 배치 전용 수집 모듈 `infrastructure:crawling`이 `bbq`(BBQ 메뉴)·`admdongkor`(행정동 경계) 2모듈로 분할돼 24개가 됐고, 메일·SMS 모듈 `infrastructure:messaging`이 채널 모듈 `mail`·`sms`와 벤더 모듈 `javamail`·`solapi` 4모듈로 분할돼 27개가 됐습니다. 이후 `infrastructure:payment`가 채널 모듈 `infrastructure:pg`로 리네임되고 벤더 구현 `infrastructure:tosspayments`가 신설돼 **28개**가 됐습니다. 읽기 계약은 전부 `application` 모듈이 소유합니다 — 공유 계약 55개를 잠시 `domain`이 갖고 있었으나 application 모듈 통합으로 그 근거(앱 간 수평 의존 회피)가 사라져 되돌렸고, 그 결과 `com.tastyhouse.application` 최상위 패키지를 한 모듈이 단독으로 갖습니다. **코어는 이후 역할에 맞춰 `infrastructure:http-client`를 거쳐 `infrastructure:restclient`로 리네임됐고, `WebClient`/webflux를 전면 제거해 Spring `RestClient`로 통일했으며, 예외 계약(`ExternalApiException`/`ExternalApiErrorCode`)도 완전히 해체해 도메인 `ErrorCode`로 흡수했습니다**(모듈 수 24는 불변 — 리네임과 HTTP 클라이언트 전환·예외 계약 해체는 모듈 경계를 바꾸지 않습니다).

  `infrastructure`는 기술별로 나뉜 **중첩 프로젝트**입니다(디렉터리 `backend/infrastructure/{persistence,redis,restclient,file-storage,firebase,aws-s3,aws-ses,aws-sns,oauth,pg,tosspayments,mail,javamail,sms,solapi,bbq,admdongkor}/`, Gradle 좌표는 각각 `:infrastructure:{이름}`. `restclient`는 `infrastructure:external` → `infrastructure:http-client`를 거쳐 리네임됐습니다). 17개 중 `file-storage`를 뺀 16개가 driven(아웃바운드) 어댑터 모듈이고(채널 모듈 `mail`·`sms`·`pg`는 포트 구현 없이 도메인 서비스 빈 등록과 벤더 조립을 맡습니다), `file-storage`만 자바 코드 없이 파일 저장 벤더 구현(현재 firebase)을 묶어 앱에 노출하는 조립 전용 스타터입니다. 자바 패키지는 persistence·redis가 `com.tastyhouse.infrastructure..`로 불변이고, **코어 `restclient`는 리네임과 함께 자기 패키지도 `com.tastyhouse.restclient..`로 옮겼습니다.** **벤더·채널 13모듈(자바 코드가 있는 나머지 — `file-storage`는 자바 코드가 없어 소유할 패키지 자체가 없습니다)은 예외로 `com.tastyhouse.external..`을 그대로 유지합니다**(모듈마다 하위 패키지가 겹치지 않게 나눠 소유) — `InfrastructureModuleConfig`가 `com.tastyhouse.infrastructure`를 통째로 스캔해서, 그 아래로 옮기면 빈 스캔 범위가 어긋나 admin/ceo/batch 부팅이 깨지기 때문입니다(코어는 그 스캔 트리에 걸리는 벤더 빈이 없어 이 제약에서 자유롭습니다).

  외부 연동 15모듈은 **앱별 실사용 차이**로 나뉩니다 — `restclient`(HTTP 코어: Boot `RestClient.Builder` customizer만 — 예외·에러코드는 없다. 파일 저장과는 무관 — 벤더 구현이 도메인 포트 `FileStoragePort`를 직접 구현합니다. 구 `infrastructure:external` → `infrastructure:http-client`를 거쳐 리네임, `WebClient`/webflux는 전면 제거하고 Spring `RestClient`로 통일했습니다) / `file-storage`(파일 저장 스타터 — 코드 없이 벤더 구현 firebase를 조립, **4앱이 직접 의존하는 유일한 파일 저장 좌표**) / `firebase`(앱이 아니라 `file-storage`가 의존) / `oauth`(web만) / `pg`(결제 PG 채널(web만) — 라우터 + tosspayments 조립) / `mail`·`sms`(web만 — 메일·SMS 채널 모듈. 각각 벤더 `javamail`·`solapi`를 `runtimeOnly`로 조립하고 `MailDomainConfig`·`SmsDomainConfig`를 가집니다. `file-storage`와 달리 코드가 있습니다) / `javamail`·`solapi`·`tosspayments`(앱이 아니라 채널 모듈이 의존 — `tosspayments`는 앱이 아니라 `pg`가 의존) / `bbq`·`admdongkor`(batch만) / `aws-s3`·`aws-ses`·`aws-sns`(**어느 앱도 의존하지 않고 컴파일만 검증** — provider 기본값이 전부 비-AWS. 전환 절차는 각각 `backend/infrastructure/aws-s3/AGENTS.md`·`backend/infrastructure/aws-ses/AGENTS.md`·`backend/infrastructure/aws-sns/AGENTS.md`). 외부 연동 실패 코드(SMS·메일·행정동 경계 등)는 이 15모듈이 아니라 도메인 `ErrorCode`가 소유합니다. 모듈 지도와 배치 기준은 `backend/CLAUDE.md`의 "모듈 지도" 절을 참조합니다.
