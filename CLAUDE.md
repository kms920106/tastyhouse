...
- **backend 모듈** (`backend/settings.gradle`): 총 14개이며 세 갈래입니다.
  ...
  - **공유 모듈**: `domain-module`, `infrastructure:persistence`, `infrastructure:redis`, `infrastructure:external`, `security-module`, `logging-module`, `api-common-module`.
  ...
  `infrastructure`는 기술별로 나뉜 **중첩 프로젝트**입니다(디렉터리 `backend/infrastructure/{persistence,redis,external}/`, Gradle 좌표 `:infrastructure:persistence`·`:infrastructure:redis`·`:infrastructure:external`). 셋 다 driven(아웃바운드) 어댑터입니다. 자바 패키지는 persistence·redis가 `com.tastyhouse.infrastructure..`로 불변이고, **`infrastructure:external`만 예외로 `com.tastyhouse.external..`을 그대로 유지합니다** — `InfrastructureModuleConfig`가 `com.tastyhouse.infrastructure`를 통째로 스캔해서, 그 아래로 옮기면 OAuth 빈 제외 필터가 우회돼 admin/ceo/batch 부팅이 깨지기 때문입니다. 모듈 지도와 배치 기준은 `backend/CLAUDE.md`의 "모듈 지도" 절을 참조합니다.
