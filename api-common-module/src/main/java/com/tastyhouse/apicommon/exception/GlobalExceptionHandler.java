 ...
 *
 * <p><b>web-api는 이 핸들러를 쓰지 않는다.</b> web-api는 {@code ExternalApiException}(소셜 로그인·PG 등
 * 외부 연동)·{@code NoHandlerFoundException}·{@code MissingServletRequestParameterException}·
 * {@code MethodArgumentTypeMismatchException} 핸들러를 추가로 갖고, 검증 실패 메시지 형식도
 * {@code "필드명: 메시지"}로 달라 응답 계약이 다르다. 이는 우연한 차이가 아니라 소비자별 계약 차이이므로
 * 통합하지 않고 web-api가 자체 핸들러를 유지한다({@code com.tastyhouse.webapi.exception.GlobalExceptionHandler}).
 * 그래서 {@code WebApiApplication}의 {@code scanBasePackages}는 이 패키지를 제외하고
 * {@code com.tastyhouse.apicommon.file}만 스캔한다.
 */
@Slf4j
