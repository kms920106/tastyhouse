 ...
 */
final class MemberPointHistoryMapper {

    private MemberPointHistoryMapper() {
    }

    /**
     * JPA 엔티티를 도메인 모델로 재구성한다(조회 경로).
     */
    static MemberPointHistory toDomain(MemberPointHistoryJpaEntity entity) {
        return MemberPointHistory.reconstitute(
            entity.getId(),
            entity.getMemberId(),
            entity.getPointType(),
     ...
     */
    static MemberPointHistoryJpaEntity toEntity(MemberPointHistory domain) {
        return MemberPointHistoryJpaEntity.create(
            domain.getMemberId(),
            domain.getPointType(),
