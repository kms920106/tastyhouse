...
- 어댑터: `infrastructure-module/.../product/persistence/`(8개 애그리거트 × `JpaEntity`/`Mapper`/`JpaRepository`, `RepositoryImpl`은 `Product`+7개 하위 애그리거트로 9개, 총 33개 파일). `ProductRepositoryImpl`은 미분리 `file`의 `QUploadedFile`(core 생성)과 이미 이동한 `shop`의 `QShopJpaEntity`(infra 생성)를 조인.
