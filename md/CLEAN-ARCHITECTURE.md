...
> 추가일: 2026-07-19
> 상태: **notice 파일럿 완료, admin 전환 완료, banner 전환 완료, bug 전환 완료, faq 전환 완료, coupon 전환 완료, event 전환 완료, member 전환 완료(코어 3개 애그리거트만; follow/referral 제외), member follow·referral 전환 완료(follow는 상태전이 없는 insert+delete-only, referral은 상태전이(`reward`/`cancel`) 있는 애그리거트), partnership 전환 완료, policy 전환 완료, point 전환 완료, rank 전환 완료(하드→소프트 삭제 전환 포함), reservation 전환 완료(@Version 낙관적 락 애그리거트 분리 최초 사례), search 전환 완료(3개 애그리거트, 읽기 전용 애그리거트 reconstitute-only 최초 사례), review 전환 완료(6개 애그리거트 + 죽은 코드 애그리거트 ReviewProduct 삭제), shop 전환 완료(@Entity 17개 중 핵심 `Shop` 애그리거트 1개만 우선 전환 — 최초의 부분 전환 사례, 나머지 16개 자식 엔티티는 core 잔류; `QShop` 소멸로 `order`/`product`/`shop`(`ShopChoiceRepositoryImpl`) 및 이미 이동한 `review`까지 4개 파일이 함께 깨져 `PathBuilder`로 전환한 이번 프로젝트 최대 팬아웃 사례), shop 자식 16개 엔티티 후속 전환 완료(부분 전환을 완결한 사례 — `ShopAmenity`/`ShopAmenityCategory`/`ShopBannerImage`/`ShopBookmark`/`ShopBreakTime`/`ShopBusinessHour`/`ShopChoice`/`ShopClosedDay`/`ShopFoodType`/`ShopFoodTypeCategory`/`ShopOrderMethod`/`ShopOwnerMessageHistory`/`ShopPhotoCategory`/`ShopPhotoCategoryImage`/`Station`/`Tag`를 순수 POJO + `infrastructure/shop/persistence`의 JpaEntity/Mapper로 분리하고 JpaRepository 15개·RepositoryImpl 4개(`ShopDetailRepositoryImpl`/`ShopChoiceRepositoryImpl`/`ShopBookmarkRepositoryImpl`/`TagRepositoryImpl`)를 infrastructure-module로 이동; `ShopChoiceRepositoryImpl`은 이동 후 기존 `PathBuilder<Object>("ShopJpaEntity")` 우회를 같은 패키지의 정식 `QShopJpaEntity` 조인으로 복원(order 선례와 동일 패턴); `ShopRepositoryImpl`(자식 Q-type 6종 import)과 `ReviewRepositoryImpl`(`QStation` import)은 infra→infra 참조로 전환; core 애플리케이션 계층이 직접 주입하던 `ShopBookmarkJpaRepository`/`StationJpaRepository`와 `new ShopBookmark(...)` 생성을 신설 `StationRepository`(`existsById`)·확장된 `ShopBookmarkRepository`(`existsById`/`save`)·`ShopBookmark.of(...)` 팩토리로 대체해 core의 JPA 직접 의존을 제거; JPA 연관관계는 처음부터 전무(raw FK + QueryDSL join)했고 `ShopBookmark`만 `@Convert` FK VO(`MemberId`) 보유), order 전환 완료(연관관계 없는 3개 애그리거트 `Order`+`OrderProduct`+`OrderProductOption`; 미전환 `payment`가 쓰는 `OrderIdConverter`는 core 잔류, 미사용 컨버터 2개는 삭제; order 이동으로 `shop` 전환 때 생겼던 `PathBuilder` 우회를 `QShopJpaEntity` 정식 조인으로 복원; 이미 이동한 `review`의 `QOrderProduct`→`QOrderProductJpaEntity` 치환), payment 전환 완료(연관관계 없는 3개 애그리거트 `Payment`+`PaymentRefund`+`TossPaymentRecord`; order를 먼저 분리해야만 풀리는 core→infra 역참조 blocker가 있었던 최초 사례 — order 미분리 시 `OrderRepositoryImpl`이 core의 `QPayment`를 조인 중이라 payment를 먼저 옮기면 컴파일이 깨져 order→payment 순서로 진행, order 이동 후 `OrderRepositoryImpl`의 `QPayment`→`QPaymentJpaEntity` import 치환으로 해소; `TossPaymentRecordMapper`는 파라미터 50여 개라 3중 대조 필요), verification 전환 완료(연관관계 없는 2개 애그리거트 `EmailVerification`+`PhoneVerification`; `@Embedded` VO(`VerificationCode`, 공유 `PhoneNumber`)는 그대로 유지, 이 프로젝트 최초로 `BaseEntity`를 상속하지 않는 사례 — 감사 필드가 수동 `created_at`뿐이고 `updated_at`이 없음; 기존 detached merge save를 load-copy-save로 교정하고, 파사드 `web-api`의 `AuthPasswordResetService`가 도메인 모델을 직접 다뤄 그곳에도 명시적 save 추가), product 전환 완료(연관관계 없는 8개 애그리거트 `Product`+`ProductBbq`+`ProductCategory`+`ProductCommonOption`+`ProductCommonOptionGroup`+`ProductImage`+`ProductOption`+`ProductOptionGroup`; `shop`/`review`가 먼저 분리되어야만 풀리는 core→infra 역참조 blocker가 있었던 두 번째 사례(payment 이후) — `shop`의 `ShopChoiceRepositoryImpl`과 `review`의 `ReviewRepositoryImpl`이 core의 `QProduct`/`QProductImage`를 직접 조인 중이라 product를 먼저 옮기면 컴파일이 깨져 shop·review→product 순서로 진행, 두 파일 모두 `QProductJpaEntity`/`QProductImageJpaEntity`로 치환해 해소; `Product`만 `@Embedded ProductDiscountInfo` VO 보유(공유 아님 — `PhoneNumber` 선례처럼 `@Embeddable` 어노테이션 유지한 채 JpaEntity가 그대로 재사용), 감사 필드 전부 미소비로 8개 애그리거트 모두 `createdAt`/`updatedAt` 생략), file 전환 완료(연관관계·QueryDSL·update/soft delete가 없는 최소 CRUD 변형이면서, 코드베이스에서 QueryDSL 조인으로 가장 많이 참조되는 hub 도메인이었던 사례 — 참조자 14개 `RepositoryImpl`(order/shop×3/member/member-follow/product×2/review/rank×3/event/banner)이 `QUploadedFile`을 조인 중이라, 그 참조자들이 먼저 전부 infra로 이동해 core의 `QUploadedFile` 참조가 0건이 된 뒤에야 안전하게 전환할 수 있었던 마지막 순번 사례; 참조자 14개의 `QUploadedFile`(core 생성) import를 `QUploadedFileJpaEntity`(infra 생성)로 전부 치환(모두 infra→infra 참조))** — 이후 도메인은 아래 롤아웃 절차로 점진 적용
...
### member follow·referral 전환 결과물 (reference — member 코어 후속 전환, 연관관계 없음·`@Convert` FK VO만)

- **범위**: member 코어 전환 시 제외됐던 두 하위 도메인 `member/follow`(`MemberFollow`)·`member/referral`(`MemberReferral`)을 전환. 둘 다 JPA `@OneToMany`/`@ManyToOne` 연관관계가 없고, member 코어와 동일하게 `@Convert(MemberIdConverter)` + raw 컬럼(FK VO)만 사용.
- **`MemberFollow`** — 상태전이 없는 insert+delete-only 애그리거트: 순수 모델 `core-module/.../member/follow/domain/model/MemberFollow`(`of`/`reconstitute`; 감사 시각을 소비하는 result가 없어 `createdAt`/`updatedAt` 생략, 정렬은 `MemberFollowJpaEntity`의 `BaseEntity.createdAt`으로 그대로 수행). 어댑터 `infrastructure-module/.../member/follow/persistence/{MemberFollowJpaEntity, MemberFollowMapper, MemberFollowJpaRepository, MemberFollowRepositoryImpl}` — `applyChanges` 없음(save는 insert 전용). `MemberFollowRepositoryImpl`은 member 코어 전환 때 이미 생겼던 `PathBuilder<Object>("MemberJpaEntity")` 우회를 그대로 유지하고, 자신의 Q타입만 `QMemberFollow`(도메인) → `QMemberFollowJpaEntity`(infra)로 치환(`viewerFollow` alias 포함). `delete(MemberFollow)`는 detached 도메인 POJO를 `jpaRepository.delete(entity)`에 넘기는 대신 `deleteById(memberFollow.getId())`로 구현. 명시적 save 규칙: `MemberFollowCommandService`가 이미 생성 시 `save`, 삭제 시 `delete` 호출 중이라 추가 조치 없음(admin/search 선례와 동일). 순수 단위 테스트: `core-module/src/test/.../member/follow/domain/model/MemberFollowTest`.
- **`MemberReferral`** — 상태전이(`reward`/`cancel`, enum `MemberReferralStatus` `EnumType.STRING`+`columnDefinition="VARCHAR(20)"`) 있는 애그리거트: 순수 모델 `core-module/.../member/referral/domain/model/MemberReferral`(`register`/`reconstitute`; `MemberReferralResult.from`이 `createdAt`만 소비해 `updatedAt`은 생략, coupon/point/reservation과 동형의 비대칭 사례). 어댑터 `infrastructure-module/.../member/referral/persistence/{MemberReferralJpaEntity, MemberReferralMapper, MemberReferralJpaRepository, MemberReferralRepositoryImpl}` — `QMemberReferral`(도메인) → `QMemberReferralJpaEntity`(infra) 치환, 크로스 도메인 참조 없음. **save 재구현(load-copy-save)**으로 `reward`/`cancel` 상태 변경을 `applyChanges`가 복사.
  - **⚠️ `ReferralCommandService#register` 재할당 수정**: 분리 전에는 `MemberReferral.register(...)` → `save(referral)`(더티 체킹 시절엔 같은 인스턴스가 계속 유효) → `referral.reward()` → `save(referral)` 순서였다. POJO 전환 후 `save`는 `toDomain`으로 **새 도메인 인스턴스**(id 부여됨)를 반환하므로, 첫 `save` 호출 결과를 `referral = memberReferralRepository.save(referral);`로 반드시 재할당해야 한다. 재할당 없이 원본(`id=null`) 인스턴스에 `reward()` 후 다시 save하면 update가 아닌 **중복 insert**가 발생하는 회귀였다(이번 전환에서 발견해 수정).
...
### product 전환 결과물 (reference — 8개 애그리거트, 크로스 도메인 core→infra 역참조 blocker 두 번째 사례, `@Embedded` VO 비공유 유지)

- 순수 모델: `core-module/.../product/domain/model/Product`·`ProductBbq`·`ProductCategory`·`ProductCommonOption`·`ProductCommonOptionGroup`·`ProductImage`·`ProductOption`·`ProductOptionGroup`(`of`/`reconstitute`, 전부 JPA 연관관계 없이 raw FK(`Long shopId`/`productId`/`optionGroupId` 등)로만 연결). 8개 애그리거트 전부 감사 시각을 어떤 result도 소비하지 않아 `createdAt`/`updatedAt` 미보유(admin/search 선례와 동형). `ProductImage`는 update 행위가 없는 불변 애그리거트라 전 필드 `final`.
- **`@Embedded` VO 비공유 유지**: `Product`만 보유한 `ProductDiscountInfo`(`discount_price`/`discount_rate` 2컬럼)는 다른 도메인과 공유되지 않는 product 전용 VO이지만, `member`의 `PhoneNumber` 선례와 동일하게 `@Embeddable`/`@Column` 어노테이션을 그대로 유지한 채 `core-module/.../product/domain/vo/ProductDiscountInfo`에 남기고 순수 POJO `Product`와 `ProductJpaEntity` 양쪽이 동일 타입으로 재사용한다(공유 여부와 무관하게 "VO가 실제로 JPA 임베디드로 쓰이면 어노테이션을 유지한다"는 원칙 확인 사례).
- **크로스 도메인 core→infra 역참조 blocker(두 번째 사례, payment 이후)**: product 분리 전 조사에서 아직 미전환이던 `shop`의 `ShopChoiceRepositoryImpl`과 `review`의 `ReviewRepositoryImpl`이 core의 `QProduct`/`QProductImage`(도메인 모델 Q타입)를 직접 join하고 있어, product를 먼저 옮기면 그 Q타입이 core에서 사라져 두 파일이 컴파일 실패하는 구조였다. 이후 `shop`·`review`가 각각 먼저 infrastructure-module로 전환되며 두 파일 자체가 infra로 이동했고, product 전환 시점에는 두 파일의 `QProduct`/`QProductImage` import를 `QProductJpaEntity`/`QProductImageJpaEntity`(product의 infra Q타입)로 치환하는 것이 infra→infra 참조가 되어 문제없이 해소됐다(`shop`→`review`→`product` 순서가 강제된 사례; order→payment 선례와 동일 패턴의 반복 적용).
- 어댑터: `infrastructure-module/.../product/persistence/`(8개 애그리거트 × `JpaEntity`/`Mapper`/`JpaRepository`, `RepositoryImpl`은 `Product`+7개 하위 애그리거트로 9개, 총 33개 파일). `ProductRepositoryImpl`은 (당시) 미분리 `file`의 `QUploadedFile`(core 생성)과 이미 이동한 `shop`의 `QShopJpaEntity`(infra 생성)를 조인 — 이후 `file` 전환으로 `QUploadedFileJpaEntity`(infra 생성)로 치환됨(아래 file 전환 결과물 참고).
- 명시적 save: `ProductCommandService`의 8개 커맨드 메서드(`createProduct`/`createProductCategory`/`saveProductImage`/`saveProductBbq`/`saveProductOptionGroup`/`saveProductOption`/`updateProduct`/`markSoldOut`/`deactivateProduct`/`markBbqOptionsSynced`)와 `ProductReviewEventListener#updateProductReviewStats`가 분리 이전부터 이미 전부 `save` 호출 중이라 신규 추가 없음(admin 선례와 동일, 더티 체킹 의존 지점 0건).
- 순수 단위 테스트: `core-module/src/test/.../product/domain/model/ProductTest`·`ProductBbqTest`·`ProductCategoryTest`·`ProductCommonOptionTest`·`ProductCommonOptionGroupTest`·`ProductImageTest`·`ProductOptionTest`·`ProductOptionGroupTest`

### file 전환 결과물 (reference — 최소 CRUD 변형 + hub 도메인 최종 전환 사례)

- 순수 모델: `core-module/.../file/domain/model/UploadedFile`(`of`/`reconstitute`, JPA 연관관계 없음, 행위 메서드 없음, insert-only). 감사 필드 `createdAt`/`updatedAt`는 어떤 result도 소비하지 않지만 `UPLOADED_FILE` 테이블에 두 컬럼이 실재하므로 매핑 일관성을 위해 포함(admin/product처럼 생략하지 않은 예외적 선택).
- **hub 도메인 최종 전환**: `file`은 이 프로젝트에서 QueryDSL 조인으로 가장 많이 참조되던 도메인으로, 전환 시점에 core `QUploadedFile`을 static import하던 `RepositoryImpl`이 14개(order/shop×3/member/member-follow/product×2/review/rank×3/event/banner)나 있었다. core-module은 infrastructure-module을 의존할 수 없어(의존 방향 역전 금지) file을 먼저 전환하면 이 14개가 전부 컴파일 실패하는 구조였으므로, 참조자 14개가 각각 자기 도메인 전환을 통해 먼저 infrastructure-module로 이동을 완료하고 core의 `QUploadedFile` 참조가 0건이 될 때까지 file 전환을 보류했다(hub 도메인은 참조자를 먼저 옮기고 마지막에 전환하는 순서가 강제되는 최초이자 가장 큰 팬아웃 사례 — order/product의 두 역참조 blocker보다 큰 규모).
- 어댑터: `infrastructure-module/.../file/persistence/`(`UploadedFileJpaEntity`/`UploadedFileMapper`/`UploadedFileJpaRepository`/`UploadedFileRepositoryImpl` — QueryDSL 없이 순수 pass-through, `save`는 update 경로가 없어 insert 전용).
- **참조자 14개 Q타입 치환**: file 전환으로 core `QUploadedFile`이 사라지고 infra `QUploadedFileJpaEntity`가 생성되면서, 위 14개 `RepositoryImpl`(이미 전부 infra에 있음)의 `QUploadedFile` import를 `QUploadedFileJpaEntity`로 일괄 치환. 전부 infra→infra 참조라 정식 Q타입을 직접 사용(PathBuilder 우회 불필요) — order가 payment/shop 전환 후 겪은 "이동 후 정식 Q타입 복원" 패턴과 동일. `ShopDetailRepositoryImpl`은 커스텀 별칭 인스턴스 `activeFile`/`inactiveFile`(`new QUploadedFile(...)`)의 타입도 `QUploadedFileJpaEntity`로 함께 치환.
- 명시적 save 규칙은 대상 없음 — `FileCommandService#save`가 분리 이전부터 이미 저장 시 `save` 호출 중(admin 선례와 동일, 더티 체킹 의존 지점 0건).
- 순수 단위 테스트: `core-module/src/test/.../file/domain/model/UploadedFileTest`
