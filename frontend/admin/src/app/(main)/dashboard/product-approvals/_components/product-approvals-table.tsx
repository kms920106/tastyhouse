...
import type { ProductApprovalsTableMeta } from "./product-image-review-columns";
...
/** 두 탭이 컬럼만 다르고 페이징·빈 상태 처리는 같으므로 테이블은 한 벌로 공용한다. */
export function ProductApprovalsTable<TRequest>({
  table,
  isPending,
}: {
  table: TableType<TRequest>;
  isPending: boolean;
}) {
  ...
  const totalElements = (table.options.meta as ProductApprovalsTableMeta<TRequest>).totalElements;
    ...
    <div className="flex flex-1 flex-col gap-4">
      <div className="overflow-x-auto">
        ...
        >
          ...
          <TableBody>
            {isPending ? (
              Array.from({ length: pageSize }, (_, index) => `product-approval-row-skeleton-${index}`).map((key) => (
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} className="border-border/60 hover:bg-white/2.5">
                  {row.getVisibleCells().map((cell) => (
                    <TableCell key={cell.id} className="px-3 py-4 align-top" style={{ width: cell.column.getSize() }}>
        ...
        <div className="flex items-center gap-4 text-muted-foreground text-sm">
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              disabled={isPending}
            >
              <SelectTrigger size="sm" className="w-20" id="product-approvals-rows-per-page">
