...
import type { ShopRiderGuidesTableMeta } from "./shop-rider-guides-columns";
...
export function ShopRiderGuidesTable({
  table,
  isPending,
}: {
  table: TableType<ShopRiderGuideListItem>;
  isPending: boolean;
}) {
  ...
  const totalElements = (table.options.meta as ShopRiderGuidesTableMeta).totalElements;

  return (
    <div className="flex flex-1 flex-col gap-4">
      {/* 좁은 화면에서 페이지 자체가 가로 스크롤되지 않도록 테이블만 스크롤 컨테이너에 둔다. */}
      <div className="overflow-x-auto">
        ...
        >
          ...
          <TableBody>
            {isPending ? (
              Array.from({ length: pageSize }, (_, index) => `rider-guide-row-skeleton-${index}`).map((key) => (
              ...
              <TableRow>
                <TableCell colSpan={table.getVisibleLeafColumns().length} className="h-24 text-center">
                  {SHOP_RIDER_GUIDE_ADMIN_COPY.EMPTY_TITLE}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
        ...
        <div className="flex items-center gap-4 text-muted-foreground text-sm">
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              disabled={isPending}
            >
              <SelectTrigger size="sm" className="w-20" id="shop-rider-guides-rows-per-page">
