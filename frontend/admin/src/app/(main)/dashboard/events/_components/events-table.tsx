...
import type { EventsTableMeta } from "./events-columns";
...
export function EventsTable({ table, isPending }: { table: TableType<EventListItem>; isPending: boolean }) {
  ...
  const totalElements = (table.options.meta as EventsTableMeta).totalElements;
    ...
    <div className="flex flex-1 flex-col gap-4">
      <div>
        ...
        >
          ...
          <TableBody>
            {isPending ? (
              Array.from({ length: pageSize }, (_, index) => `event-row-skeleton-${index}`).map((key) => (
