...
// (`availability/error.tsx` 와 같은 판단 — 이 라우트군의 일부 error.tsx 는 `retry` 를 쓰고 있어
// 실제로 재시도가 동작하지 않으므로 따라 하지 않는다).
export default function MenuBoardError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  React.useEffect(() => {
    console.error(error);
  }, [error]);
