 ...
 * {@code MemberPointJpaEntity} + {@code MemberPointMapper}가 담당한다. 도메인이
 * 프레임워크-프리이므로 변경 후 저장은 더티 체킹이 아니라 command 서비스가 명시적으로
 * {@code MemberPointRepository#save}를 호출해야 한다.
 */
@Getter
public class MemberPoint {
    ...
    private MemberPoint(Long id, MemberId memberId, Integer availablePoints, Integer expiredThisMonth) {
        this.id = id;
    ...
    public static MemberPoint of(MemberId memberId) {
        return new MemberPoint(null, memberId, 0, 0);
    }
     ...
     */
    public static MemberPoint reconstitute(Long id, MemberId memberId, Integer availablePoints, Integer expiredThisMonth) {
        return new MemberPoint(id, memberId, availablePoints, expiredThisMonth);
    }
