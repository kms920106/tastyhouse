...
public class ReservationQueryService {
    ...
    public DailySlotAvailabilityResult findSlotAvailability(Long shopId, LocalDate date, MemberId memberId) {
        ...
        Map<LocalTime, ReservationSlot> slotByTime = slotRepository.findByShopAndDate(shopId, date).stream()
            .collect(Collectors.toMap(ReservationSlot::getSlotTime, Function.identity()));
        ...
        List<SlotAvailability> slots = SlotPolicy.allSlots().stream()
            .map(time -> {
                ReservationSlot slot = slotByTime.get(time);
                int remaining = slot != null ? slot.remaining() : SlotPolicy.CAPACITY_PER_SLOT;
