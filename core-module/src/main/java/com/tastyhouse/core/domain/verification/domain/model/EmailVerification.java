 ...
 * {@code EmailVerificationJpaEntity} + {@code EmailVerificationMapper}가 담당한다. 도메인이
 * 프레임워크-프리이므로 변경 후 저장은 더티 체킹이 아니라 호출부가 명시적으로
 * {@code EmailVerificationRepository#save}를 호출해야 한다.
 */
@Getter
public class EmailVerification {
    ...
    private final String email; // 인증 이메일 주소
    private final VerificationCode verificationCode; // 인증 코드 (6자리)
    private EmailVerificationStatus status; // 인증 상태
    ...
    /**
     * 신규 이메일 인증을 생성한다. 아직 영속되지 않았으므로 식별자는 없다.
     */
    ...
    public static EmailVerification reconstitute(
        ...
        return new EmailVerification(id, email, verificationCode, status, expiresAt, verifiedAt, createdAt);
    }
