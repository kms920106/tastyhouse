...
class ReviewTagTest {
    ...
    void of_createsTransientTag() {
        ReviewTag tag = ReviewTag.of(1L, 50L);

        assertThat(tag.getId()).isNull();
        assertThat(tag.getReviewId()).isEqualTo(1L);
        assertThat(tag.getTagId()).isEqualTo(50L);
    }

    @Test
    @DisplayName("reconstitute는 DB 상태로부터 식별자를 포함해 재구성한다")
    void reconstitute_restoresPersistedState() {
        ReviewTag tag = ReviewTag.reconstitute(4L, 1L, 50L);
