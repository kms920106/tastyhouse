...
import type { OrdersTableMeta } from "./orders-columns";
...
export function OrdersTable({ table, isPending }: { table: TableType<OrderListItem>; isPending: boolean }) {
  ...
  const totalElements = (table.options.meta as OrdersTableMeta).totalElements;
    ...
    <div className="flex flex-1 flex-col gap-4">
      <div>
        ...
        >
          ...
          <TableBody>
            {isPending ? (
              Array.from({ length: pageSize }, (_, index) => `order-row-skeleton-${index}`).map((key) => (
