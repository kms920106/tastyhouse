...
export function AmenityCategoriesSheet({ open, onOpenChange }: AmenityCategoriesSheetProps) {
  ...
  const loadCategories = React.useCallback(() => {
    ...
    void fetchAmenityCategoriesAction().then((result) => {
      if (!active) return;
      setIsLoading(false);
      if (result.success && result.data) {
        setCategories(result.data);
      } else {
        setError(result.message ?? SHOP_MESSAGE.AMENITY_CATEGORIES_LOAD_FAILED);
      }
    });
  ...
  function startEdit(category: AmenityCategory) {
  ...
  const onSubmit = (values: AmenityCategoryFormValues) => {
    startTransition(async () => {
      const { success, message } = editingId
        ? await updateAmenityCategoryAction(editingId, values)
        : await createAmenityCategoryAction(values);

      if (success) {
        toast.success(
          editingId ? SHOP_MESSAGE.AMENITY_CATEGORY_UPDATE_SUCCESS : SHOP_MESSAGE.AMENITY_CATEGORY_CREATE_SUCCESS,
        );
        cancelEdit();
        loadCategories();
      } else {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="flex w-full flex-col sm:max-w-md">
        <SheetHeader>
          <SheetTitle>편의시설 카테고리 관리</SheetTitle>
          <SheetDescription>전역 편의시설 마스터 카테고리를 등록하고 수정합니다.</SheetDescription>
        </SheetHeader>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-4">
          <div className="space-y-2">
            ...
            {error ? (
                ...
                {categories.map((category) => (
                  ...
                  >
                    <div className="flex flex-col">
                      ...
                      <span className="text-muted-foreground text-xs">{category.amenity}</span>
                    </div>
              ...
              <p className="text-muted-foreground text-sm">등록된 편의시설 카테고리가 없습니다.</p>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            <form id="amenity-category-form" noValidate onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
              <Controller
                control={form.control}
                name="amenity"
                render={({ field, fieldState }) => (
                  <Field className="gap-1.5" data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor="amenity-category-amenity">편의시설 종류</FieldLabel>
                    <Select value={field.value} onValueChange={field.onChange} disabled={busy}>
                      <SelectTrigger id="amenity-category-amenity" className="w-full" aria-invalid={fieldState.invalid}>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectGroup>
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field className="gap-1.5" data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor="amenity-category-display-name">노출명</FieldLabel>
                    <Input
                      {...field}
                      id="amenity-category-display-name"
                      placeholder="예: 와이파이"
                      maxLength={AMENITY_DISPLAY_NAME_MAX}
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field className="gap-1.5" data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor="amenity-category-active-image">활성 이미지</FieldLabel>
                    <Input
                      id="amenity-category-active-image"
                      type="file"
                      accept="image/*"
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field className="gap-1.5" data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor="amenity-category-inactive-image">비활성 이미지</FieldLabel>
                    <Input
                      id="amenity-category-inactive-image"
                      type="file"
                      accept="image/*"
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field className="w-24 gap-1.5" data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor="amenity-category-sort">정렬</FieldLabel>
                    <Input
                      id="amenity-category-sort"
                      type="number"
                      value={field.value}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Field orientation="horizontal">
                    <FieldLabel htmlFor="amenity-category-visible">노출 여부</FieldLabel>
                    <Switch
                      id="amenity-category-visible"
                      checked={field.value}
