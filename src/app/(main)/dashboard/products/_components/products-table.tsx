...
import type { ProductsTableMeta } from "./products-columns";
...
export function ProductsTable({ table, isPending }: { table: TableType<ProductListItem>; isPending: boolean }) {
  ...
  const totalElements = (table.options.meta as ProductsTableMeta).totalElements;
    ...
    <div className="flex flex-1 flex-col gap-4">
      <div>
        ...
        >
          ...
          <TableBody>
            {isPending ? (
              Array.from({ length: pageSize }, (_, index) => `product-row-skeleton-${index}`).map((key) => (
