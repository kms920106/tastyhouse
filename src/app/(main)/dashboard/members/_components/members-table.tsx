...
import type { MembersTableMeta } from "./members-columns";
...
export function MembersTable({ table, isPending }: { table: TableType<MemberListItem>; isPending: boolean }) {
  ...
  const totalElements = (table.options.meta as MembersTableMeta).totalElements;
    ...
    <div className="flex flex-1 flex-col gap-4">
      <div>
        ...
        >
          ...
          <TableBody>
            {isPending ? (
              Array.from({ length: pageSize }, (_, index) => `member-row-skeleton-${index}`).map((key) => (
