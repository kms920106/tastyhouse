...
| File | Description |
|------|-------------|
| `app-config.ts` | Exports `APP_CONFIG` — app `name` ("Tastyhouse CEO"), `version` (read from `package.json`), `copyright` (with current year), and `meta.title`/`meta.description` used for HTML metadata |
