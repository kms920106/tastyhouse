...
/**
 * infrastructure:messaging 모듈의 auto-configuration — 메일(JavaMail)·SMS(Solapi) 발송 채널.
 *
 * <p>메일·SMS 인증은 사용자 앱에서만 쓰므로 web-api만 이 모듈을 의존한다. 클래스패스 존재만으로
 * 활성화된다. 분리 전에는 persistence의 {@code MailDomainConfig}·{@code SmsDomainConfig}가
 * {@code MailSender}·{@code SmsSender} 빈을 무조건 요구해서 admin/ceo/batch도 발송 어댑터를
 * 강제로 들여와야 했다. 두 설정을 이 모듈로 이관해 그 결합을 끊었다 — 아웃바운드 포트 구현이
 * 일부 앱에만 있으면 그 포트를 구현하는 모듈이 도메인 서비스 빈을 등록한다(근거는 {@code AGENTS.md}).
 *
