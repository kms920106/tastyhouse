...
**결과.** `external`은 `config/`·`exception/`만 남은 순수 HTTP 코어가 됐다. `infrastructure:firebase`는 external 의존을 끊어 `domain` + firebase-admin만 갖고, 스타터 `infrastructure:file-storage`는 `runtimeOnly project(':infrastructure:firebase')` 한 줄만 조립한다. 그 결과 파일 저장만 쓰는 **admin-api·ceo-api의 런타임 클래스패스에서 external과 webflux(reactor-netty)가 빠진다.** web은 oauth·payment·messaging을, batch는 crawling을 경유해 external을 계속 갖는다. **이후 3분할(2026-09-26)로 옛 `infrastructure:aws`는 `aws-s3`·`aws-ses`·`aws-sns` 3모듈로 나뉘었다** — `aws-ses`·`aws-sns`는 SES/SNS가 `ExternalApiException`/`ExternalApiErrorCode`를 쓰므로(`MailProperties`는 messaging 소유라 `aws-ses`가 그쪽도 의존하는 이유다) external 의존을 유지하고, `aws-s3`는 domain + spring-cloud-aws-starter-s3만 가져 external 의존이 없다.
