...
/**
 * infrastructure:external 코어 모듈의 진입점 설정 — 벤더 중립 공용 자산만 담는다.
 *
 ...
 * 각각 별도 모듈로 분리됐고, 앱은 실제로 쓰는 모듈만 의존하고 {@code @Import} 한다
 * ({@code FirebaseModuleConfig}·{@code AwsModuleConfig}·{@code OAuthModuleConfig}·
 * {@code PaymentModuleConfig}·{@code MessagingModuleConfig}·{@code CrawlingModuleConfig}).
 *
