...
public interface ProductAllergenJpaRepository extends JpaRepository<ProductAllergenJpaEntity, Long> {
     ...
     * {@code uk_product_allergen_product_type}(product_id + allergen_type)을 위반하지 않는다.
     *
     ...
     * 같은 성분을 다시 체크한 교체에서 항상 중복 키로 실패한다({@code ShopDeliveryTipTierJpaRepository}가
     * 같은 이유로 같은 형태를 쓴다). {@code clearAutomatically}는 삭제된 행이 1차 캐시에 남아 뒤이은
     * 조회를 오염시키지 않도록 한다.
     */
