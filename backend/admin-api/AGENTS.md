...
- `infrastructure:file-storage` — 파일 저장 스타터(챕터 03). `infrastructure:firebase`(도메인 포트 `FileStoragePort` 구현 — 파일 업로드)를 묶어 전이로 공급하므로 **앱은 벤더 모듈을 직접 선언하지 않는다**. 코어 `infrastructure:external`(WebClient·외부 연동 예외)은 파일 저장 SPI가 삭제된 뒤 스타터의 조립 대상에서 빠져 **이 앱의 `runtimeClasspath`에 없다**(webflux도 함께 빠졌다). **OAuth·결제·메시징 모듈은 의존하지 않는다** — 관리자 화면에는 소셜 로그인·PG 결제·메일/SMS 발송 유스케이스가 없다
