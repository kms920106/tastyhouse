...
| File | Description |
|------|-------------|
| `build.gradle` | web + springdoc 의존. **`application`**·`infrastructure:persistence`·`infrastructure:external`·`infrastructure:firebase`·`infrastructure:redis`·`logging-module`·`security-module`·`api-common-module`을 `implementation`으로 참조. 외부 연동은 **코어(`external`)와 파일 저장 구현(`firebase`) 둘뿐**이며, OAuth·결제·메일/SMS 어댑터는 web-api 전용이라 이 모듈에 오지 않는다. QueryDSL 의존은 없다 |
...
- `infrastructure:firebase` — `FileStorageStrategy` 구현(파일 업로드). **OAuth·결제·메시징 모듈은 의존하지 않는다** — 관리자 화면에는 소셜 로그인·PG 결제·메일/SMS 발송 유스케이스가 없다
