...
| File | Description |
|------|-------------|
| `build.gradle` | web + springdoc 의존. `domain-module`·**`application`**·`infrastructure:persistence`·`external-api`·`logging-module`·`security-module`·`api-common-module`을 `implementation`으로 참조. QueryDSL 의존은 없다 |
