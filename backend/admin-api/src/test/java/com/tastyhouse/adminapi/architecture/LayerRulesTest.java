...
class LayerRulesTest {
     ...
     * {@link AdminApp} 등 마커 애노테이션이므로 규칙도 마커로 판정한다.
     *
     * <p>세 갈래로 나눠 검사한다.
     * <ul>
     *   <li><b>(a) UseCase 인터페이스</b> — {@code ..port.in..}의 인터페이스에 의존한다면 그것이
     *       {@link AdminApp}를 달고 있어야 한다. 마커를 인터페이스가 직접 가지므로 술어가 단순하다.</li>
     *   <li><b>(b) Command record</b> — record에는 마커가 없다. 소속을 {@link AppOwnership#derive}로
     *       <b>유도</b>해 그 집합이 {@link AdminApp}인지 본다(유도 규칙은 그 클래스 Javadoc 참조).</li>
