 ...
 *
 * <p><b>챕터 03 개정</b> — application 계층이 admin-application 모듈로 물리 분리되면서 이 파일은
 ...
 * 은 전부 {@code admin-application}의 동명 테스트로 이동했다 — 이 모듈에 남겨 두면 대상 0건으로
 * <b>공허하게 통과</b>하기 때문이다.
 *
 * <p>잔류 규칙 중 application을 가리키던 패턴은 신규 패키지 기준으로 갱신했다
 * ({@code ..application.service..} → {@code com.tastyhouse.adminapplication..service..}).
 *
 * <p>web-api에 있던 {@code shouldDependOnOauthSpiOnlyNotProviderPackages}는 이 모듈에 두지 않는다 —
 * admin에는 소셜 로그인이 없어 대상 0건으로 <b>공허하게 통과</b>하기 때문이다(전환 전 admin-api
...
class LayerRulesTest {
     ...
     * 페이징 계약 참조는 위반이 아니라 정상 경로다({@code PageResult#map} 호출 26건).
     *
     ...
     *
     * <p><b>챕터 03 개정</b> — {@code response/}는 QueryService가 조립하는 표현 계약이라
     * admin-application으로 함께 이동했으므로, 이 모듈에 남은 대상은 {@code ..request..}뿐이다.
     * Response 쪽 동일 규칙은 admin-application이 담당한다.
     */
    @Test
     ...
     *
     * <p><b>챕터 03 개정</b> — 구체 서비스가 admin-application으로 이동했으므로 이제 이 규칙 위반은
     * 애초에 컴파일되지 않는다(그 모듈의 {@code service} 패키지를 import해야 하므로). 그럼에도
     * 접미어 기준으로 남겨 두는 이유는, 누군가 구체 서비스를 admin-api로 되돌리는 시도가 곧바로
     * 드러나게 하기 위해서다.
     */
    @Test
     ...
     *
     * <p><b>챕터 03 개정</b> — 대상 패키지를 {@code ..application.service..}에서
     * {@code com.tastyhouse.adminapplication..service..}로 갱신했다. 접미어가 아니라 <b>위치</b>로 잡으므로
     ...
     *
     * <p>챕터 03의 물리 분리가 <b>되돌려지지 않았음</b>을 고정하는 규칙이다. 위 규칙들은 "컨트롤러가
     * 무엇을 주입하는가"를 보지만, 누군가 admin-api 안에 {@code @Service} 빈을 새로 만들어 application
     * 로직을 되살리는 것은 잡지 못한다.
     *
     ...
     *
     * <p><b>이 규칙은 챕터 01이 없앤 컴파일 게이트를 대체한다.</b> 그전까지 admin의 어댑터가 다른 앱의
     * UseCase를 주입하는 것은 <b>빌드가</b> 막았다 — 이 모듈의 build.gradle에
     * {@code project(':admin-application')} 하나만 있었으므로 다른 앱의 패키지는 클래스패스에 아예
     * 없었다. 챕터 01이 4개 application 모듈을 {@code :application} 하나로 합치면서 4개 앱 패키지가
     * <b>전부 이 모듈의 컴파일 클래스패스에 들어왔고</b>, 이제 admin-api이
     ...
     * 자기 앱({@code com.tastyhouse.adminapplication..})은 정방향이므로 목록에서 제외한다.
     */
    @Test
