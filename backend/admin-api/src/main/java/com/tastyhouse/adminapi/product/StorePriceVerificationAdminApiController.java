...
import com.tastyhouse.adminapi.product.request.StorePriceVerificationRejectRequest;
import com.tastyhouse.adminapi.product.request.StorePriceVerificationSearchRequest;
import com.tastyhouse.adminapi.product.response.StorePriceVerificationDetailResponse;
import com.tastyhouse.adminapi.product.response.StorePriceVerificationListItemResponse;
...
public class StorePriceVerificationAdminApiController {
    ...
    private final StorePriceVerificationCommandService storePriceVerificationCommandService;

    public StorePriceVerificationAdminApiController(
        StorePriceVerificationQueryService storePriceVerificationQueryService,
        StorePriceVerificationCommandService storePriceVerificationCommandService
    ) {
        this.storePriceVerificationQueryService = storePriceVerificationQueryService;
        this.storePriceVerificationCommandService = storePriceVerificationCommandService;
    }
    ...
    public ResponseEntity<ApiResponse<Void>> startStorePriceVerificationReview(@PathVariable Long id) {
        storePriceVerificationCommandService.startReview(id);
    ...
    public ResponseEntity<ApiResponse<Void>> approveStorePriceVerification(@PathVariable Long id) {
        storePriceVerificationCommandService.approve(id);
        return ResponseEntity.ok(ApiResponse.success(null));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> rejectStorePriceVerification(
        ...
        storePriceVerificationCommandService.reject(id, request.rejectReason());
        return ResponseEntity.ok(ApiResponse.success(null));
    }
}
