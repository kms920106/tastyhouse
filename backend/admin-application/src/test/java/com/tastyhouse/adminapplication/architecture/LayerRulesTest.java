package com.tastyhouse.adminapplication.architecture;
...
/**
 * admin-application 레이어 경계 규칙(ArchUnit).
 *
 * <p>챕터 03로 admin-api의 application 계층({@code <ctx>/{port.in,service}} · 표현 계약 {@code response})이
 * 이 모듈로 물리 분리됐다. admin-api에 있던 규칙 중 application 계층을 대상으로 하던 것들이 여기로
...
class LayerRulesTest {
    ...
    private final JavaClasses classes = new ClassFileImporter()
        ...
        .importPackages("com.tastyhouse.adminapplication");

    /**
     * 챕터 07으로 이 모듈이 소유하게 된 admin 단독 읽기 계약(split package).
     *
    ...
    /**
     * CQRS 교차 주입 금지(명령 → 조회). admin-api에서 이동한 규칙이다.
     *
    ...
    /**
     * CQRS 교차 주입 금지(조회 → 쓰기). admin-api에서 이동한 규칙이다.
     *
     * <p>{@code *QueryService}는 domain-module의 write 포트를 주입하지 않는다 — 조회 트랜잭션
     * ({@code readOnly = true})에서 쓰기 경로가 열리는 것을 구조적으로 막는다.
     *
     * <p>{@code AdminQueryService} 제외는 admin-api에서 그대로 승계한 <b>확정 carve-out</b>이다 —
    ...
    /**
     * {@code *CommandService}는 인바운드 포트를 최소 1개 구현한다. admin-api에서 이동한 규칙이다.
     *
    ...
    /**
     * {@code *QueryService}는 인바운드 포트를 최소 1개 구현한다. admin-api에서 이동한 규칙이다.
     */
    @Test
    ...
    /**
     * Command record는 경계 타입만 싣는다. admin-api에서 이동한 규칙이다(carve-out 2건 그대로 유지).
     *
    ...
    /**
     * Command record의 <b>필드</b>로 {@code MultipartFile}을 싣지 않는다. admin-api에서 이동한 규칙이다.
     *
    ...
    /**
     * 인바운드 포트는 web 플럼빙을 알지 않는다. admin-api에서 이동한 규칙이다.
     */
    @Test
    ...
    /**
     * {@code *CommandService}는 {@code ..request..}를 알지 않는다. admin-api에서 이동한 규칙이다.
     *
     * <p>물리 분리 후에는 {@code request/}가 admin-api에 남아 컴파일 자체가 불가능하므로 이 규칙은
    ...
    /**
     * QueryDSL 차단. admin-api에서 이동한 회귀 방어 규칙이다.
     */
    @Test
    ...
    /**
     * infrastructure 차단. admin-api에서 이동한 회귀 방어 규칙이다.
     *
     ...
     *
     * <p>물리 분리로 비로소 표현 가능해진 규칙이다. admin-api에 함께 있을 때는 컨트롤러가 정당하게
     ...
     * 하다: {@code TokenService}·{@code JwtTokenProvider}·{@code CustomUserDetails}·
     * {@code AdminUserDetailsService}는 Spring Security core와 JWT 타입만 쓰는 <b>서블릿-프리</b>
     * 타입이라 이 모듈로 함께 왔고, 서블릿 결합 타입(필터·EntryPoint·{@code JwtConfig}·
     * {@code RedisRepositoryConfig}·{@code PublicPaths}·{@code SecurityConfig})은 admin-api에 남았다.
     *
     * <p>{@code TokenService}·{@code AuthCommandService}가 web-api의 동명 타입과 중복되는 것은
     * <b>의도된 앱별 중복</b>이다(인증 주체 {@code Admin}·{@code ADMIN_*} ErrorCode·
     * {@code JWT_SECRET_ADMIN} 분리 — backend/CLAUDE.md 허용 목록). 통합하지 않는다.
     */
    @Test
     ...
     *
     * <p>의존 방향은 admin-api → admin-application 한 방향뿐이다. 이 규칙이 없으면 서비스가
     * 컨트롤러·Request record를 다시 끌어와 물리 분리가 이름만 남는다.
     */
    @Test
    void applicationMustNotDependOnAdapters() {
        ArchRule rule = noClasses()
            .should().dependOnClassesThat().resideInAPackage("com.tastyhouse.adminapi..")
            .because("application은 인바운드 어댑터(admin-api)를 역참조하지 않는다");
     ...
     * 컴파일 에러였다 — 프레임워크-프리가 빌드 게이트로 강제되던 것이다. 챕터 07으로 이 모듈에
