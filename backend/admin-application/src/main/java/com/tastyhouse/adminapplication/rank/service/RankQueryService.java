 ...
 * 랭킹 관리 조회 서비스(admin).
 *
 * <p>읽기 포트({@link RankManagementQueryPort})만 주입해 조회하고 Response를 조립한다(패턴 2/3). 도메인
...
public class RankQueryService implements RankQueryUseCase {
    ...
    public List<RankMemberListItemResponse> getMemberRankList(String type, int limit) {
        ...
        return rankManagementQueryPort.findMemberRanks(rankType, baseDate, limit).stream()
    ...
    public List<RankPeriodListItemResponse> getPeriods() {
        return rankManagementQueryPort.findAllPeriods().stream()
    ...
    public RankPeriodDetailResponse getPeriod(Long id) {
        RankPeriodResult period = rankManagementQueryPort.findPeriodById(RankPeriodId.of(id))
    ...
    public List<RankPrizeListItemResponse> getPrizesByPeriod(Long periodId) {
        return rankManagementQueryPort.findPrizesByPeriodId(RankPeriodId.of(periodId)).stream()
    ...
    public RankPrizeDetailResponse getPrize(Long prizeId) {
        RankPrizeManagementResult prize = rankManagementQueryPort.findPrizeById(RankPrizeId.of(prizeId))
