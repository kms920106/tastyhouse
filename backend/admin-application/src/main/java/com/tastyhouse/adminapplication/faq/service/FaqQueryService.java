 ...
 * FAQ 관리 조회 서비스.
 *
 * <p>읽기 포트({@link FaqManagementQueryPort})만 주입해 조회하고 Response를 조립한다. write 포트를
...
public class FaqQueryService implements FaqQueryUseCase {
    ...
    public FaqQueryService(FaqManagementQueryPort faqManagementQueryPort) {
        this.faqManagementQueryPort = faqManagementQueryPort;
    }

    @Override
    public List<FaqCategoryResponse> getCategories() {
        return faqManagementQueryPort.findAllCategories().stream()
    ...
    public FaqCategoryResponse getCategory(Long categoryId) {
        FaqCategoryManagementResult categoryDetail = faqManagementQueryPort.findCategoryDetailById(categoryId)
    ...
    public PaginationResponse<FaqListItemResponse> getFaqs(Long categoryId, String question, Boolean visible, int page, int size) {
        ...
        PageResult<FaqListItemResponse> pageResult = faqManagementQueryPort.findAllFaqs(condition, pageQuery)
    ...
    public FaqDetailResponse getFaq(Long id) {
        FaqDetailResult faqDetail = faqManagementQueryPort.findFaqDetailById(id)
