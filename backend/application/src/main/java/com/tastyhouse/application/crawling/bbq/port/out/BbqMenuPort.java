 ...
 *
 * <p>구현은 infrastructure:crawling의 {@code BbqMenuAdapter}다. 소비 앱이 batch 하나뿐이므로 읽기 계약
 ...
 * infrastructure:crawling의 wire DTO({@code BbqMenuResponse} 등, {@code @JsonProperty}로 BBQ API 응답
 * 스키마에 결박된 타입)를 직접 받아 변환했다. 그러면 BBQ가 응답 필드명을 바꿀 때 파장이
 * application까지 올라온다. 변환을 어댑터로 내리면 wire DTO는 infrastructure:crawling 내부 협력 타입으로
