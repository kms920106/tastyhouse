...
- **도메인 포트가 있는 기술 어댑터** — 그것은 외부 연동 모듈(`infrastructure:external` 코어 + `infrastructure:{firebase,aws-s3,aws-ses,aws-sns,oauth,payment,messaging,crawling}` 어댑터)이나 `infrastructure:persistence`(DB 어댑터) 소관이다.
