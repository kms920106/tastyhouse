...
- **출력 포트는 `<ctx>/domain/port/`에 둔다**: 외부 시스템을 도메인이 인터페이스로 선언하고 `external-api`가 구현한다(`file/domain/port/FileStoragePort`, `payment/domain/port/PgPaymentGateway`, `mail/domain/port/MailSender`, `sms/domain/port/SmsSender`, `product/domain/port/ProductReviewStatisticsPort`, `rank/domain/port/MemberReviewCountPort`). 이벤트 발행 포트는 `shared/event/DomainEventPublisher`이며 스프링 구현은 infrastructure-module의 `SpringDomainEventPublisher`다.
