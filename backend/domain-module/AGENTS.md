...
- **조회 결과 DTO는 이 모듈에 두지 않는다**: 과거 `application/dto/result`의 Result record(`@QueryProjection` 포함)와 `SearchCondition`은 전부 `infrastructure:persistence`의 `<ctx>/query/`로 이관되었다. 접미어 `Result` 통일·`Dto` 금지·admin 충돌 시 `Management` 한정어 규칙은 그 위치에서 계속 적용된다 — 상세는 `infrastructure-module/AGENTS.md` 참고.
