 ...
 * 만들어 query DAO의 raw {@code Long} 조인·투영이 깨지는 문제). 도메인 경계에서 VO가 필요한 지점은
 * {@link #adminDongIdValue()}로 승격해서 쓴다.
 *
...
public record OrderDeliveryDestination(
    ...
    /**
     * 행정동 ID를 VO로 승격해 돌려준다 — 도메인 서비스·계산기가 {@code AdminDongId}를 요구하는 지점용.
     * 행정동 매칭에 실패한 주소는 {@code null}이다.
     */
    public AdminDongId adminDongIdValue() {
        return this.adminDongId == null ? null : AdminDongId.of(this.adminDongId);
    }
}
