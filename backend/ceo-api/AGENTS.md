...
- **presentation(인바운드 어댑터) 레이어만 담당한다 (챕터 04 개정)**: 컨트롤러는 도메인을 직접 호출하지 않고 `ceo-application`의 UseCase 포트를 통해서만 호출한다.
  ...
  - **컨텍스트 패키지는 2층 구조다**: `<ctx>/adapter/in/web/`(컨트롤러) + `<ctx>/adapter/in/web/request/`. `response/`와 `service/`·`port/in/`은 `ceo-application` 소유다.
- 새 기능 추가 시 admin과 동일한 도메인-폴더 + `request/` 컨벤션(서비스·`response/`는 `ceo-application`에), `{도메인}CommandService`/`{도메인}QueryService` CQRS 중개 계층, DTO 조립·`@ModelAttribute` 조회·`@PathVariable id` 통일·`@Schema` 문서화 규칙을 그대로 따른다. 상세·근거·예시는 루트 CLAUDE.md 및 `admin-api/AGENTS.md` 참고.
