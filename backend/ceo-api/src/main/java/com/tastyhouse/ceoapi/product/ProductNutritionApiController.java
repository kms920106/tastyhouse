...
import com.tastyhouse.ceoapi.product.request.ProductNutritionUpdateRequest;
import com.tastyhouse.ceoapi.product.request.ProductShopScopeRequest;
import com.tastyhouse.ceoapi.product.response.ProductAllergenTypeResponse;
import com.tastyhouse.ceoapi.product.response.ProductNutritionResponse;
...
public class ProductNutritionApiController {
    ...
    private final ProductNutritionCommandService productNutritionCommandService;

    public ProductNutritionApiController(ProductNutritionQueryService productNutritionQueryService, ProductNutritionCommandService productNutritionCommandService) {
        this.productNutritionQueryService = productNutritionQueryService;
        this.productNutritionCommandService = productNutritionCommandService;
    }
    ...
    public ResponseEntity<ApiResponse<Void>> updateNutrition(
        ...
        productNutritionCommandService.updateNutrition(
            userDetails.getCeoId(), request.shopId(), id,
            request.servingSize(), request.totalAmount(), request.flavor(), request.size(),
            request.calorie(), request.sugars(), request.protein(), request.saturatedFat(), request.natrium(),
            request.carbohydrate(), request.cholesterol(), request.fat(), request.transFat(), request.caffeine(),
            request.setMenu(), request.allergens()
        );
    ...
    public ResponseEntity<ApiResponse<Void>> deleteNutrition(
        ...
        productNutritionCommandService.deleteNutrition(userDetails.getCeoId(), request.shopId(), id);
        return ResponseEntity.ok(ApiResponse.success(null));
    }
}
