...
import com.tastyhouse.ceoapi.product.request.ProductShopLinkCreateRequest;
import com.tastyhouse.ceoapi.product.request.ProductShopLinkReplaceRequest;
import com.tastyhouse.ceoapi.product.request.ProductShopScopeRequest;
import com.tastyhouse.ceoapi.product.response.ProductShopLinkResponse;
...
public class ProductShopLinkApiController {
    ...
    private final ProductShopLinkCommandService productShopLinkCommandService;

    public ProductShopLinkApiController(
        ProductShopLinkQueryService productShopLinkQueryService,
        ProductShopLinkCommandService productShopLinkCommandService
    ) {
        this.productShopLinkQueryService = productShopLinkQueryService;
        this.productShopLinkCommandService = productShopLinkCommandService;
    }
    ...
    public ResponseEntity<ApiResponse<Void>> replaceShopLinks(
        ...
        productShopLinkCommandService.replaceLinks(
            userDetails.getCeoId(), request.shopId(), id, request.links()
        );
    ...
    public ResponseEntity<ApiResponse<Void>> linkToShop(
        ...
        productShopLinkCommandService.linkToShop(
            userDetails.getCeoId(), id, targetShopId, request.productCategoryId()
        );
        return ResponseEntity.ok(ApiResponse.success(null));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> unlinkFromShop(
        ...
        productShopLinkCommandService.unlinkFromShop(userDetails.getCeoId(), id, targetShopId);
        return ResponseEntity.ok(ApiResponse.success(null));
    }
}
