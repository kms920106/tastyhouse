...
import com.tastyhouse.ceoapi.product.request.ProductCategoryOrderRequest;
import com.tastyhouse.ceoapi.product.request.ProductCategoryRelocateRequest;
import com.tastyhouse.ceoapi.product.request.ProductOrderRequest;
...
public class ProductSortApiController {
    ...
    private final ProductSortCommandService productSortCommandService;

    public ProductSortApiController(ProductSortCommandService productSortCommandService) {
        this.productSortCommandService = productSortCommandService;
    }
    ...
    public ResponseEntity<ApiResponse<Void>> reorderProductCategories(
        ...
        productSortCommandService.reorderProductCategories(
            userDetails.getCeoId(), request.shopId(), request.productCategoryIds()
        );
    ...
    public ResponseEntity<ApiResponse<Void>> reorderProducts(
        ...
        productSortCommandService.reorderProducts(
            userDetails.getCeoId(), request.shopId(), request.productCategoryId(), request.productIds()
        );
        return ResponseEntity.ok(ApiResponse.success(null));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> relocateProducts(
        ...
        productSortCommandService.relocateProducts(
            userDetails.getCeoId(), request.shopId(), request.targetProductCategoryId(),
            request.productIds(), request.targetOrderedProductIds()
        );
        return ResponseEntity.ok(ApiResponse.success(null));
    }
}
