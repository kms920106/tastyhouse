...
public record ShopDeliveryAreaPolygonPreviewResponse(
    ...
    public static ShopDeliveryAreaPolygonPreviewResponse from(ShopDeliveryAreaPolygonPreviewResult result) {
        return new ShopDeliveryAreaPolygonPreviewResponse(
            ...
            result.withinAllowedRadius(),
            result.adminDongs().stream().map(ShopDeliveryAreaCandidateResponse::from).toList(),
            result.addedAdminDongs().stream().map(ShopDeliveryAreaCandidateResponse::from).toList(),
            result.removedAdminDongs().stream().map(ShopDeliveryAreaCandidateResponse::from).toList(),
            result.blockedAdminDongs().stream().map(ShopDeliveryAreaBlockedResponse::from).toList(),
