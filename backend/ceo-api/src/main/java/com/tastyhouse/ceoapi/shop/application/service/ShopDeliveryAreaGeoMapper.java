...
import com.tastyhouse.ceoapi.shop.adapter.in.web.response.GeoPointResponse;
...
final class ShopDeliveryAreaGeoMapper {
    ...
    /**
     * 요청의 링 배열을 도메인 도형으로 승격한다(조회 경로 전용 — 도형 미리보기는 챕터 03에서 command
     * 경로로 합류한다).
     */
    static GeoPolygon toPolygonFromRequest(List<List<GeoPointRequest>> rings) {
        if (rings == null || rings.isEmpty()) {
            throw new BusinessException(ErrorCode.SHOP_DELIVERY_AREA_POLYGON_INVALID);
        }

        return toPolygon(rings.stream()
            .map(ring -> ring.stream()
                .map(point -> new GeoPointCommand(point.latitude(), point.longitude()))
                .toList())
            .toList());
    }
