...
import com.tastyhouse.ceoapi.product.request.ProductOptionGroupLinkRequest;
import com.tastyhouse.ceoapi.product.request.ProductOptionGroupSortRequest;
import com.tastyhouse.ceoapi.product.response.ProductOptionGroupLinkedProductResponse;
import com.tastyhouse.ceoapi.product.response.ProductOptionGroupLinkedProductsResponse;
...
public class ProductOptionGroupLinkApiController {
    ...
    private final ProductOptionGroupLinkCommandService productOptionGroupLinkCommandService;

    public ProductOptionGroupLinkApiController(
        ProductOptionGroupQueryService productOptionGroupQueryService,
        ProductOptionGroupLinkCommandService productOptionGroupLinkCommandService
    ) {
        this.productOptionGroupQueryService = productOptionGroupQueryService;
        this.productOptionGroupLinkCommandService = productOptionGroupLinkCommandService;
    }
    ...
    public ResponseEntity<ApiResponse<Void>> linkOptionGroup(
        ...
        productOptionGroupLinkCommandService.linkOptionGroup(
            userDetails.getCeoId(), request.shopId(), id, optionGroupId
        );
    ...
    public ResponseEntity<ApiResponse<Void>> unlinkOptionGroup(
        ...
        productOptionGroupLinkCommandService.unlinkOptionGroup(
            userDetails.getCeoId(), request.shopId(), id, optionGroupId
        );
        return ResponseEntity.ok(ApiResponse.success(null));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> changeOptionGroupOrder(
        ...
        productOptionGroupLinkCommandService.changeOptionGroupOrder(
            userDetails.getCeoId(), request.shopId(), id, request.optionGroupIds()
        );
        return ResponseEntity.ok(ApiResponse.success(null));
    }
