...
class LayerRulesTest {
     ...
     *
     * <p>아래 봉인 목록은 중첩 항목을 Request record로 받고 있어 중첩 Command record 치환이 필요한
     * 파일들이다(챕터 02 §5). 규칙 전체를 끄지 않고 클래스명으로 명시적으로 제외하며, 치환이 끝나는
     * 대로 목록에서 지운다 — 목록이 비면 이 모듈의 모든 {@code *CommandService}가 규칙 대상이 된다.
     */
    @Test
    void commandServicesShouldNotDependOnRequestRecords() {
        ArchRule rule = noClasses()
            .that().haveSimpleNameEndingWith("CommandService")
            .and().haveSimpleNameNotEndingWith("ProductCommandService")
            .and().haveSimpleNameNotEndingWith("ProductPriceCommandService")
            .and().haveSimpleNameNotEndingWith("ProductShopLinkCommandService")
            .and().haveSimpleNameNotEndingWith("ShopDeliveryAreaCommandService")
            .and().haveSimpleNameNotEndingWith("ShopDeliveryTipCommandService")
            .and().haveSimpleNameNotEndingWith("ShopStorePriceVerificationCommandService")
