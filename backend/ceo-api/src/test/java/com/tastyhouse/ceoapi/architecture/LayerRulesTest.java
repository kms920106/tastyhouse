...
class LayerRulesTest {
     ...
     *
     * <p><b>챕터 09 — {@code domain.shared.page..} carve-out</b>: 챕터 06(admin)이 세운 설계를
     * ceo 앱에 확대 적용하면서 도입했다. application이 {@code PageResult}를 반환하고 컨트롤러가
     * {@code PaginationResponse.from(...)}으로 조립하는 것이 그 설계이므로, 페이징 계약 참조는
     * 위반이 아니라 정상 경로다.
     *
     ...
     * {@link CeoApp} 등 마커 애노테이션이므로 규칙도 마커로 판정한다.
     *
     * <p>세 갈래로 나눠 검사한다.
     * <ul>
     *   <li><b>(a) UseCase 인터페이스</b> — {@code ..port.in..}의 인터페이스에 의존한다면 그것이
     *       {@link CeoApp}를 달고 있어야 한다. 마커를 인터페이스가 직접 가지므로 술어가 단순하다.</li>
     *   <li><b>(b) Command record</b> — record에는 마커가 없다. 소속을 {@link AppOwnership#derive}로
     *       <b>유도</b>해 그 집합이 {@link CeoApp}인지 본다(유도 규칙은 그 클래스 Javadoc 참조).</li>
    ...
    void adaptersShouldOnlyUseOwnAppUseCases() {
        ...
        for (JavaClass adapter : classes) {
            for (JavaClass dependency : adapter.getDirectDependenciesFromSelf().stream()
                ...
                if (dependency.isInterface()) {
                    if (!dependency.isAnnotatedWith(CeoApp.class)) {
                        violations.add(adapter.getName() + " -> " + dependency.getName()
                            + " (다른 앱의 UseCase — @CeoApp가 아니다)");
                    }
                } else if (dependency.isRecord()) {
                    Set<Class<? extends Annotation>> apps = commandApps.get(dependency.getName());
                    if (apps != null && !apps.equals(Set.of(CeoApp.class))) {
                        violations.add(adapter.getName() + " -> " + dependency.getName()
                            + " (소속 앱 " + AppOwnership.describe(apps) + " — @CeoApp가 아니다)");
                    }
                }
            }
        }
        ...
        assertThat(violations)
            .as("인바운드 어댑터는 자기 앱(@CeoApp)의 application 슬라이스만 의존한다")
            .isEmpty();
    }
}
