...
class LayerRulesTest {
     ...
     *
     * <p><b>이 규칙은 챕터 01이 없앤 컴파일 게이트를 대체한다.</b> 그전까지 ceo의 어댑터가 다른 앱의
     * UseCase를 주입하는 것은 <b>빌드가</b> 막았다 — 이 모듈의 build.gradle에
     * {@code project(':ceo-application')} 하나만 있었으므로 다른 앱의 패키지는 클래스패스에 아예
     * 없었다. 챕터 01이 4개 application 모듈을 {@code :application} 하나로 합치면서 4개 앱 패키지가
     * <b>전부 이 모듈의 컴파일 클래스패스에 들어왔고</b>, 이제 ceo-api이
     ...
     * 자기 앱({@code com.tastyhouse.ceoapplication..})은 정방향이므로 목록에서 제외한다.
     */
    @Test
