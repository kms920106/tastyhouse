...
/**
 * web-application 레이어 경계 규칙(ArchUnit).
 *
 * <p>챕터 02로 web-api의 application 계층({@code <ctx>/{port.in,service}} · 표현 계약 {@code response})이
 * 이 모듈로 물리 분리됐다. web-api에 있던 규칙 중 application 계층을 대상으로 하던 것들이 여기로
...
class LayerRulesTest {
    ...
    /**
     * 챕터 08으로 이 모듈이 소유하게 된 web 단독 읽기 계약(split package).
     *
    ...
    /**
     * CQRS 교차 주입 금지(명령 → 조회). web-api에서 이동한 규칙이다.
     *
    ...
    /**
     * CQRS 교차 주입 금지(조회 → 쓰기). web-api에서 이동한 규칙이다.
     *
     * <p>{@code *QueryService}는 domain-module의 write 포트를 주입하지 않는다 — 조회 트랜잭션
     * ({@code readOnly = true})에서 쓰기 경로가 열리는 것을 구조적으로 막는다.
     *
     * <p>{@code ShopQueryService} 제외는 web-api에서 그대로 승계한 <b>확정 carve-out</b>이다 —
     * 그 write 포트는 배달팁 계산 경로가 도메인 서비스에 넘길 애그리거트를 로드하는 데 쓰므로
     * 표현용 투영이 아니라 도메인 계산 입력이다. 이 목록에 새 항목을 추가하지 않는다.
     */
    @Test
    ...
    /**
     * {@code *CommandService}는 인바운드 포트를 최소 1개 구현한다. web-api에서 이동한 규칙이다.
     *
    ...
    /**
     * {@code *QueryService}는 인바운드 포트를 최소 1개 구현한다. web-api에서 이동한 규칙이다.
     */
    @Test
    ...
    /**
     * Command record는 경계 타입만 싣는다. web-api에서 이동한 규칙이다(carve-out 2건 그대로 유지).
     *
     ...
     *
     * <p><b>챕터 10 개정 — {@code com.tastyhouse.domain.shared.page..}를 세 번째 carve-out으로 추가했다</b>
     * (챕터 06이 admin-application에 세우고 챕터 09가 ceo에 확대한 선례를 web에 마저 적용). 근거는 위
     * {@code MultipartFile} carve-out과 <b>동일한 구조</b>다: 이 규칙이 겨냥하는 것은 Command record가
     * <b>필드로</b> 도메인 모델을 싣는 것인데, ArchUnit은 같은 {@code ..port.in..} 패키지에 사는
     * <b>QueryUseCase의 메서드 시그니처</b>까지 함께 잡는다. 챕터 10으로 QueryUseCase의 목록 반환 타입이
    ...
    /**
     * Command record의 <b>필드</b>로 {@code MultipartFile}을 싣지 않는다. web-api에서 이동한 규칙이다.
     *
    ...
    /**
     * 인바운드 포트는 web 플럼빙을 알지 않는다. web-api에서 이동한 규칙이다.
     */
    @Test
    ...
    /**
     * {@code *CommandService}는 {@code ..request..}를 알지 않는다. web-api에서 이동한 규칙이다.
     *
     * <p>물리 분리 후에는 {@code request/}가 web-api에 남아 컴파일 자체가 불가능하므로 이 규칙은
    ...
    /**
     * QueryDSL 차단. web-api에서 이동한 회귀 방어 규칙이다.
     */
    @Test
    ...
    /**
     * infrastructure 차단. web-api에서 이동한 회귀 방어 규칙이다.
     *
     ...
     *
     * <p>물리 분리로 비로소 표현 가능해진 규칙이다. web-api에 함께 있을 때는 컨트롤러가 정당하게
     * 서블릿 타입을 쓰므로 모듈 전역 금지를 걸 수 없었다.
     *
     * <p>{@code MultipartFile}({@code org.springframework.web.multipart..})만 예외다 — 업로드를 받는
     * UseCase 메서드의 경계 파라미터로 존치한다(§6). 이 예외가 auth 컨텍스트 이동의 게이트이기도
     * 하다: {@code TokenService}·{@code JwtTokenProvider}·{@code CustomUserDetails}는 Spring Security
     * core와 JWT 타입만 쓰는 <b>서블릿-프리</b> 타입이라 이 모듈로 함께 왔고, 서블릿 결합 타입
     * (필터·EntryPoint·{@code JwtConfig}·{@code SecurityConfig})은 web-api에 남았다.
     */
    @Test
     ...
     *
     * <p>의존 방향은 web-api → web-application 한 방향뿐이다. 이 규칙이 없으면 서비스가
     * 컨트롤러·Request record를 다시 끌어와 물리 분리가 이름만 남는다.
     */
    @Test
    void applicationMustNotDependOnAdapters() {
        ArchRule rule = noClasses()
            .should().dependOnClassesThat().resideInAPackage("com.tastyhouse.webapi..")
            .because("application은 인바운드 어댑터(web-api)를 역참조하지 않는다");
     ...
     * 컴파일 에러였다 — 프레임워크-프리가 빌드 게이트로 강제되던 것이다. 챕터 08으로 이 모듈에
    ...
    /**
     * <b>챕터 10 신설</b> — application 계층은 Swagger를 알지 않는다(챕터 06이 admin, 챕터 09가 ceo에
     * 심은 규칙의 web 확대이자 <b>3개 앱 완결</b>).
     *
     * <p>챕터 10 이전에는 {@code *QueryService}가 {@code @Schema}가 붙은 {@code *Response}를 직접
     * 조립했고, 이 모듈에서만 {@code io.swagger} import가 <b>131개 파일</b>(응답 record 131개와 정확히
     * 일치 — 서비스 오염은 없었다)이었다. 유스케이스 계층이 <b>HTTP 표현 포맷과 API 문서화 도구를 아는
     * 상태</b>였다는 뜻이다. 챕터 10이 Response를 web-api로 승격하고 유스케이스는 프레임워크-프리
    ...
    void applicationShouldNotDependOnSwagger() {
        ArchRule rule = noClasses()
            ...
            .because("유스케이스 계층은 API 문서화 도구를 알지 않는다(Response 조립은 web-api 담당)");

        rule.check(classes);
    }

    /**
     * <b>챕터 10 신설</b> — application 계층은 표현 모듈(api-common-module)을 알지 않는다.
     *
     * <p>{@code PaginationResponse}·{@code ApiResponse} 같은 HTTP 래퍼는 표현 계약이다. 이 모듈이
     * 그것을 조립하던 것이 챕터 10이 걷어낸 두 번째 위반이며(페이징은 이제 {@code PageResult}로
     * 반환하고 컨트롤러가 {@code PaginationResponse.from(...)}으로 감싼다), 이 규칙이 재발을 막는다.
     * 승격 전 이 모듈의 {@code PaginationResponse} 사용은 28개 파일이었다.
     *
     * <p><b>{@code build.gradle}의 api-common 의존 제거는 챕터 11의 몫이다</b> — 이 챕터는 소스 레벨
     * 참조만 0으로 만들었으므로, 지금은 이 규칙이 <b>유일한</b> 방어선이다(ceo와 같은 상태이며, admin은
     * 의존 제거가 1차 방어선이고 규칙이 회귀 방어인 이중화 상태라는 점이 다르다).
     */
    @Test
