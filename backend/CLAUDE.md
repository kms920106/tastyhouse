...
| `infrastructure:restclient`(구 `infrastructure:external` → `infrastructure:http-client`를 거쳐 개명)과 벤더·채널 9모듈(`firebase`·`aws-s3`·`aws-ses`·`aws-sns`·`oauth`·`payment`·`messaging`·`bbq`·`admdongkor`) | domain `<ctx>/port/` 구현(파일·PG·메일·SMS) + 자체 SPI(소셜 로그인·크롤링·행정동 경계). 코어(`restclient`)가 공통 자산(`RestClient.Builder` customizer뿐 — 예외·에러코드는 없다)을 갖고, 파일 저장 벤더 구현 `firebase`·`aws-s3`를 뺀 나머지 7모듈이 그것을 의존한다(`firebase`·`aws-s3`는 domain 포트만 구현하므로 코어 의존이 없다) |
