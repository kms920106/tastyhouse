...
| `infrastructure:redis` | `RedisModuleAutoConfiguration` (`com.tastyhouse.infrastructure.redis`, `RedisModuleConfig` 리네임) | 스캔 `com.tastyhouse.infrastructure.redis` | `@AutoConfiguration(before = RedisAutoConfiguration.class)` — **스펙과 다른 실측 결론(아래 "함정 1" 참고)** |
| `security-module` | `SecurityModuleAutoConfiguration` (`com.tastyhouse.security`, `SecurityModuleConfig` 리네임) | 스캔 `com.tastyhouse.security` + `@EnableConfigurationProperties(JwtProperties)` | `@ConditionalOnWebApplication(SERVLET)` |
...
- **함정 2 — 앱이 직접 참조하는 라이브러리 타입은 명시 선언이 필요해진다**: web/admin/ceo 3개 앱의 `config/jwt/RedisRepositoryConfig`가 앱별 키 접두사(`admin:rt:` 등)를 주입하려고 `StringRedisTemplate`을 **직접 타입 참조**한다. `infrastructure:redis`가 `implementation`이던 시절에는 이 타입이 전이로 컴파일 클래스패스에 들어왔지만, `runtimeOnly`로 내리면서 그 전이가 끊겨 **컴파일이 깨졌다.** 해결은 `infrastructure:redis` 자체를 되돌리는 것이 아니라, 3개 앱의 `build.gradle`에 `implementation 'org.springframework.boot:spring-boot-starter-data-redis'`를 **명시 선언**하는 것이다 — 라이브러리 모듈(어댑터 구현)은 여전히 `runtimeOnly`로 숨기되, 앱이 실제로 참조하는 서드파티 타입만 그 타입을 제공하는 라이브러리를 직접 선언한다. **일반화**: 라이브러리 모듈을 `runtimeOnly`로 내릴 때, 그 모듈 안의 어떤 타입이든 앱 코드가 직접 import하고 있다면 컴파일이 깨진다 — 그 타입이 그 모듈만의 고유 타입이 아니라 표준 서드파티 타입(`StringRedisTemplate` 등)이면, 앱이 그 서드파티 라이브러리를 직접 선언해 해결하고, 우리 자체 타입(어댑터 구현 클래스 등)이면 애초에 앱이 그것을 참조하는 것 자체가 설계 위반이므로 참조를 없앤다.
