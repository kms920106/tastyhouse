...
web-api ──┬─→ application (implementation)             ← 컨트롤러가 자기 앱의 UseCase 포트를 주입
          ...
          ├─→ infrastructure:{file-storage,oauth,payment,mail,sms} (runtimeOnly) ← 실사용 어댑터만
...
infrastructure:solapi    ─→ infrastructure:restclient, domain        ← SmsSender 구현
   ※ 벤더(javamail·solapi·aws-ses·aws-sns)는 채널 모듈을 의존하지 않는다 — 채널이 벤더를 runtimeOnly로 조립하므로 역방향은 순환
...
- 실행 가능한(bootJar) 모듈은 `web-api`/`admin-api`/`ceo-api`/`batch-module` 넷뿐이며, **모듈 재편으로도 이 넷과 산출물 이름은 바뀌지 않았다**(라이브러리 모듈만 추가됐다). 나머지(`domain`/`application`/`infrastructure:persistence`/`infrastructure:redis`/`infrastructure:{external,firebase,aws-s3,aws-ses,aws-sns,oauth,payment,mail,javamail,sms,solapi,bbq,admdongkor}`/`security-core`/`security-module`/`api-common-module`/`logging-module`)는 `bootJar` 비활성 + plain jar.
