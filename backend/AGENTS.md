...
| `web-application/` | web-api의 **application 계층**(컨텍스트별 인바운드 포트 + CQRS 서비스 + auth 토큰·principal). **`response/`는 챕터 10으로 web-api로 승격됐다**(`io.swagger` 0건). infra를 컴파일 클래스패스에 두지 않는다 (see `web-application/AGENTS.md`) |
