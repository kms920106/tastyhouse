...
| `infrastructure-module/` | domain 포트의 어댑터 — `<ctx>/persistence`(write: JPA/매퍼) + `<ctx>/query`(read: QueryDSL QueryDao + Result DTO) + `<ctx>/listener` + 도메인 서비스 빈 등록(`<ctx>/config/<Ctx>DomainConfig`) (see `infrastructure-module/AGENTS.md`) |
