...
| File | Description |
|------|-------------|
| `build.gradle` | `application`·`infrastructure:persistence`·`infrastructure:external`·`infrastructure:firebase`·`infrastructure:oauth`·`infrastructure:payment`·`infrastructure:messaging`·`infrastructure:redis`·`security-module`·`logging-module`·`api-common-module`을 모두 `implementation`으로 의존 + web, webflux, security, data-redis, aop, validation, JJWT, springdoc (p6spy는 logging-module이 api로 전이). QueryDSL 의존은 없다 |
