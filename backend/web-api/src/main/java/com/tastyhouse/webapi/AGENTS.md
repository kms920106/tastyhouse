...
- **외부 API 호출은 외부 연동 모듈 어댑터로 위임** — OAuth는 `infrastructure:oauth`, 결제는 `infrastructure:payment`, 메일/SMS는 채널 모듈 `infrastructure:mail`·`infrastructure:sms`(각각 벤더 `javamail`·`solapi`를 묶는다), 파일 업로드는 `infrastructure:file-storage`(스타터 — `FileStoragePort` 구현 `infrastructure:firebase`를 묶는다). 외부 수집(`infrastructure:bbq`·`infrastructure:admdongkor`)은 batch-module 전용이라 이 모듈과 무관하다.
