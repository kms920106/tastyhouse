...
- **QueryDSL·infra 직접 호출 금지 (개정)**: `src/main`에 `com.querydsl.*` import·`@QueryProjection` 선언·`com.tastyhouse.infrastructure..` import가 **전면 0건**이며, `src/test/.../architecture/LayerRulesTest`(ArchUnit)의 `shouldNotDependOnInfrastructureQuery`가 봉인 목록 없이 이를 차단한다. 조회는 `application-common-module`의 `{Ctx}QueryPort` 인터페이스를 주입해 쓴다(reference: `product` 도메인의 `ProductQueryPort#findFirstBbqSyncTarget` — BBQ 옵션 동기화 대상 조회). 이 모듈만 클래스명 `*CommandService`/`*QueryService`인 CQRS 서비스가 0개(`XxxSchedulerService` 네이밍)라, `applicationServicesShouldNotDependOnWebLayer` 규칙은 매칭 대상에 `*SchedulerService`를 포함시켜 공허 통과를 없앴다(`allowEmptyShould(true)`는 이 규칙 하나에만 유지 — 그 외 batch 규칙은 전부 순수 강제).
