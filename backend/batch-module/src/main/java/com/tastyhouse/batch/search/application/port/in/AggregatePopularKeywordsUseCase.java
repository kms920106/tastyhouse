package com.tastyhouse.batch.search.application.port.in;

/**
 * search 배치 잡의 인바운드 포트.
 *
 * <p>{@code @Scheduled} 트리거(adapter/in/scheduler)가 이 인터페이스만 주입하고, 구현은
 * {@code SearchKeywordSchedulerService}가 맡는다 — 트리거가 구체 서비스를 알지 않아야 잡 본문의 교체·테스트 대역
