...
- **command/DTO는 지역 변수로 추출 후 넘긴다(인자 자리 인라인 조립 지양)**: `Xxx.of(...)` 팩토리 결과를 core command 서비스 호출 인자에 인라인으로 바로 넘기지 않고, 먼저 `Xxx command = Xxx.of(...);`로 추출한 뒤 `service.method(command)`(또는 `service.method(XxxId.of(id), command)`)로 전달한다. 조립과 호출을 문장 단위로 분리해 가독성·디버깅·도메인 간 형태 일관성을 높이기 위함이다. 이미 지역 변수로 조립되던 `SearchCondition`과 짧은 식별자 승격 `XxxId.of(id)`(인자 자리 허용)·응답 변환 `XxxResponse.from(...)`은 대상 아님. 대체된 인라인 한 줄을 `//` 주석으로 남기지 않는다(reference: `NoticeService#createNotice`·`#updateNotice`, 동일 적용 `banner`·`bug`·`coupon`·`policy`·`admin` Facade). 상세는 루트 CLAUDE.md 참고.
