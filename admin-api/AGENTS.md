...
컨트롤러가 `core-module`에 직접 결합되는 것을 막기 위해, 도메인별로 admin-api 소속 application 서비스를 두어 컨트롤러와 core 사이를 중개한다(과거 이 계층을 "Facade"로 부르던 문서 표현은 정정되었다). 이 서비스가 도메인 호출과 DTO↔admin-api Request/Response 변환을 전담하므로, 컨트롤러는 `com.tastyhouse.core.*`를 import하지 않고 admin-api 타입(Service·Request·Response)에만 의존한다.

**도메인당 CQRS 분리 (진행 중 전환)**: core-module → domain-module 전환에 따라, 단일 `{도메인}Service` 파사드는 도메인당 두 서비스로 **분해**된다 — `{도메인}CommandService`(`@Transactional`, domain write 포트·도메인 서비스만 주입, 생성/수정/삭제/상태전이)와 `{도메인}QueryService`(`@Transactional(readOnly = true)`, infrastructure-module의 `{도메인}QueryDao`만 주입, 조회 + Response 조립 private 매퍼). 컨트롤러는 필요한 서비스를 각각 주입하고, 조회만 있는 도메인은 QueryService만 둔다. CommandService는 `..query..`를, QueryService는 write 포트를 서로 주입하지 않는다. **QueryDSL은 절대 쓰지 않는다** — `com.querydsl..`·`..infrastructure..persistence..` 의존은 `architecture/LayerRulesTest`(ArchUnit)가 차단하며, infra 중 `..query..`만 import한다. reference 구현: `notice/NoticeCommandService`·`notice/NoticeQueryService`. 아직 전환되지 않은 도메인은 기존 단일 `{도메인}Service` 형태로 남아 있고, 각 도메인 작업 시 전환한다.
...
### Common Patterns
- 컨트롤러 → 도메인 application 서비스(`{도메인}CommandService`/`{도메인}QueryService`) → domain write 포트 또는 infra `{도메인}QueryDao`. 컨트롤러는 HTTP 매핑만, 서비스는 위임·변환만 담당한다(전환 전 도메인은 `{도메인}Service` → core application 서비스의 2-hop 구조가 남아 있다).
