...
| Directory | Purpose |
|-----------|---------|
| `src/main/java/com/tastyhouse/adminapi/` | 관리자 컨트롤러 루트 — `common/`(공용 인프라: `ApiResponse`·`PageRequest` 등), `config/`(JWT·Security), `admin/`·`auth/`·`banner/`·`bug/`·`coupon/`·`event/`·`faq/`·`file/`·`member/`·`notice/`·`policy/`(각 도메인 폴더는 컨트롤러 + `{도메인}CommandService`/`{도메인}QueryService` + `request/`·`response/`) |
