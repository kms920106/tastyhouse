...
/**
 * security-module의 접두사 주입형 Redis 저장소를 admin-api 전용 키 접두사로 빈 등록한다.
 */
@Configuration
public class RedisRepositoryConfig {
    ...
    public RefreshTokenRedisRepository refreshTokenRedisRepository(StringRedisTemplate redisTemplate) {
        return new RefreshTokenRedisRepository(redisTemplate, "admin:rt:");
    }

    @Bean
    public BlacklistRedisRepository blacklistRedisRepository(StringRedisTemplate redisTemplate) {
        return new BlacklistRedisRepository(redisTemplate, "admin:bl:");
    }
}
