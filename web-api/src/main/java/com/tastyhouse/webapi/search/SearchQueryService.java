 ...
 *
 * <p>가게·메뉴·리뷰 검색은 다른 도메인(product/review/shop)의 read model에 위임하는데, 그 도메인들의
 * infra query DAO는 아직 신설되지 않았으므로(전환 그룹 3) 잠정적으로 기존 core 경로
 * ({@link ProductQueryService}·{@link ReviewRepository}·{@link ShopRepository})를 그대로 주입한다.
 * shop/product/review 전환 시 이 세 의존을 각 도메인의 infra query DAO 주입으로 교체해야 한다.
 */
@Service
