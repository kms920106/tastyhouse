...
- **presentation 레이어만 담당**: 컨트롤러는 core를 직접 호출하지 않고 도메인별 `{도메인}Service`를 통해서만 호출한다(과거 "Facade"로 부르던 계층과 동일하며, 클래스 네이밍은 예외 없이 `Service`로 통일되어 있다 — 상세는 루트 CLAUDE.md 참고). JPA Repository를 직접 주입하지 않는다.
  - web-api도 admin-api와 동일하게 도메인별 application 서비스를 통해 도메인을 호출한다(reference: `order/OrderService`, `member/MemberService`, `auth/AuthService`). 이 서비스가 도메인 호출과 DTO↔Request/Response 변환을 전담하며, 컨트롤러는 `com.tastyhouse.core.*`를 import하지 않는다. **CQRS 분리 전환 중**: 전환된 도메인은 `{도메인}CommandService`(domain write 포트 주입) + `{도메인}QueryService`(infra `{도메인}QueryDao` 주입)로 분해되고, 조회만 있는 도메인은 QueryService만 둔다(reference: `notice/NoticeQueryService`). QueryDSL은 쓰지 않는다 — `architecture/LayerRulesTest`가 `com.querydsl..`·`..infrastructure..persistence..` 의존을 차단하며 infra 중 `..query..`만 import한다.
