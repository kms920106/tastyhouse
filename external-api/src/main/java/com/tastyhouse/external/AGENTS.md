...
## Purpose
`domain-module`이 `<ctx>/domain/port/`에 선언한 출력 포트(`MailSender`·`SmsSender`·`FileStoragePort`·`PgPaymentGateway` 등)를 구현하는 외부 시스템 연동 어댑터. 각 제공자(provider)별로 격리된 패키지 구조로 관심사를 분리하며, 외부 API 응답 DTO가 도메인 모델로 유입되지 않도록 차단한다.
...
- **Port 구현**: `MailSender`, `SmsSender` 등 `domain-module`의 `<ctx>/domain/port/` 포트를 구현하되, 외부 응답 DTO를 도메인 모델로 직접 반환하지 않음. 포트는 프레임워크-프리이므로 WebClient/SDK 타입이 시그니처로 새어나가지 않게 한다.
